...
function buildCreationSkuColorNameInstruction({ skuSubject, targetLanguage } = {}) {
  ...
  return [
    ...
    groupedColorLabels
      ...
      : "The exact SKU color name is unavailable. Do not render any color-name label and never guess one from the background, source-card text, lighting, shadow, or shared neutral trim.",
    subjectUnitCount > 1
...
export function normalizeCreationReferenceRoles(value) {
  ...
  return entries
    .filter((entry) => cleanString(entry?.role) !== "style")
    .map((entry, index) => {
...
function buildCreationInfographicRebuildPrompt({
  ...
  return [
    ...
    buildCreationPlatformPromptInstruction(platform, industryTemplate, "infographic-rebuild"),
...
export function buildCreationPlan(input = {}) {
  ...
  const infographicRebuildItems = buildCreationInfographicRebuildItems({
    ...
    return {
      ...
      constraints: [],
      conversionIntent,
      prompt: `${item.prompt} ${buildCreationConversionIntentGuidance(conversionIntent)}`.trim(),
    };
  });
