...
function nextSkuCode(skus) {
  const codes = new Set(skus.map((sku) => asText(sku?.skuCode).toLowerCase()).filter(Boolean));
  let number = 1;
  while (codes.has(`sku-${number}`)) number += 1;
  return `SKU-${number}`;
}

function nextSkuKey(skus, value1, value2) {
  const baseKey = skuKey(value1, value2);
  const keys = new Set(skus.map((sku) => asText(sku?.key)).filter(Boolean));
  if (!keys.has(baseKey)) return baseKey;

  let number = 2;
  while (keys.has(`${baseKey}#${number}`)) number += 1;
  return `${baseKey}#${number}`;
}

export function addSkuVariant(input, variant1Value, variant2Value = "") {
  if (!input || typeof input !== "object" || Array.isArray(input) || !Array.isArray(input.skus)) {
    return { draft: input, added: false, reason: "invalid_draft" };
  }

  const value1 = asText(variant1Value);
  const hasSecondVariant = Boolean(asText(input.variants?.name2));
  const value2 = hasSecondVariant ? asText(variant2Value) : "";
  if (!value1) return { draft: input, added: false, reason: "missing_variant1" };
  if (hasSecondVariant && !value2) return { draft: input, added: false, reason: "missing_variant2" };

  const variantKey = skuKey(value1, value2);
  if (input.skus.some((sku) => skuKey(sku?.variant1Value, hasSecondVariant ? sku?.variant2Value : "") === variantKey)) {
    return { draft: input, added: false, reason: "duplicate" };
  }

  const product = input.product && typeof input.product === "object" ? input.product : {};
  const sku = {
    key: nextSkuKey(input.skus, value1, value2),
    variant1Value: value1,
    variant2Value: value2,
    skuCode: nextSkuCode(input.skus),
  ...
  return {
    draft: { ...input, skus: [...input.skus, sku] },
    added: true,
    reason: "",
  };
}
...
export function validateDraft(input, options = {}) {
  ...
  skus.forEach((sku, index) => {
    const value1 = asText(sku.variant1Value);
    const value2 = variants.name2 ? asText(sku.variant2Value) : "";
    if (!value1 || (variants.name2 && !value2)) return;
    const key = skuKey(value1, value2);
    if (variantKeys.has(key)) {
      errors.push({
        path: `skus.${index}.variant1Value`,
        code: "duplicate_variant",
        message: `SKU ${index + 1} 变种组合与第 ${variantKeys.get(key) + 1} 行重复`,
      });
    } else {
      variantKeys.set(key, index);
    }
  });
