...
test("Creation exposes clipboard batch import and the tools menu exposes the extension package", async () => {
  ...
  assert.match(html, /id="productImageImportImageViewer"[\s\S]*id="productImageImportViewerCloseButton"[\s\S]*id="productImageImportViewerImage"/);
  ...
  assert.match(controller, /media\.append\(checkbox, preview\);[\s\S]*actions\.appendChild\(zoomButton\);[\s\S]*card\.append\(media, actions\);/);
  ...
  assert.match(styles, /\.product-image-import-thumbnail\s*\{[^}]*object-fit:\s*cover/);
  assert.match(styles, /\.product-image-import-zoom-button\s*\{[^}]*width:\s*30px;[^}]*height:\s*30px;/);
  assert.match(styles, /\.product-image-import-image-viewer\s*\{[^}]*position:\s*absolute;[^}]*inset:\s*0;/);
