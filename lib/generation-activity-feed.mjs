...
export function getGenerationActivityDisplayText(detail) {
  ...
  if (prefixedDetailMatch) {
    /* 心跳只留摘要，不带明细：上游每 15 秒推来的心跳文本完全相同，
       在同一行里反复打印「仍在等待最终图，请保持页面打开」是同一件事说很多遍。
       行还在、摘要还在，所以任务仍然看得出在跑；变形图标才是这次心跳的回执。 */
    if (hasHeartbeatPrefix(text)) {
      return { summary: formatGenerationActivitySummaryLabel(prefixedDetailMatch[1]), detail: "" };
    }
