...
选中 **主题** 会完全隐藏连接字段，只显示一张独立的配色卡片：7 套界面配色按每行 4 个排列，每项只显示色块和简短名称。默认配色为「靛蓝」，其余 6 套取自中国传统色库。选择会保存在浏览器本地，并同步到嵌入的 Temu 工作台。
...
提示词页的参数区还有「质量」下拉：`auto`、`low`、`medium`、`high`（默认），以及只有 2.5 模型才有的 `xhigh` 和 `max`。选着 `xhigh` / `max` 再把工具模型切回 `gpt-image-2` 时，质量会自动降为 `high`，而不是发一个上游必然拒绝的请求。提示词生图还提供可选的「透明背景」开关，仅在路由模式和直连生图路线显示；开启后会强制输出 PNG，向上游发送 `background=transparent`，并把选择随该提示词任务一起保存。Gemini/模型协议路线不显示此开关，提示词任务保持不透明背景。
...
| 工作流 | 上传/数量限制 | 补充说明 |
| --- | --- | --- |
| 提示词生图 | 参考图最多 15 张；支持路由/直连路线请求透明背景 | 单个会话并行任务槽上限为 15；透明背景会固定 PNG，不等于上游并发额度 |
...
套图、写真、文章插图和 PPT 各自提供独立的「思考等级」与「质量」选择。思考等级会随计划与生成请求提交；质量会按当前生图模型能力归一化（2.5 模型才提供 `xhigh` 和 `max`）。入队后的套图或 PPT 会保留这两个值，用于生成、补图、失败重试和单页编辑。套图仍可选择 SKU 生成规则，但新计划固定一个 SKU 主体。
...
- 路由模式、直连模式、Gemini、主题四个互斥配置区，配合鼠标悬停/键盘聚焦说明、固定且独立滚动的生成日志，以及每行 4 个的 7 套界面配色；主题区不再提供自定义颜色和花卉点缀。
...
- 「主题」区改为每行 4 个的 7 套简短配色色块，移除自定义颜色和花卉点缀，默认配色改为「靛蓝」。
