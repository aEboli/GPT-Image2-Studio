...
function sanitizeUnsupportedEvidenceArrayPair(englishValues, chineseValues, claims) {
  const english = Array.isArray(englishValues) ? englishValues : [];
  const chinese = Array.isArray(chineseValues) ? chineseValues : [];
  const sanitizedEnglish = english.map((item) => sanitizeUnsupportedEvidenceText(item, claims));
  const keepEnglishIndex = sanitizedEnglish.map(Boolean);
  return {
    english: sanitizedEnglish.filter(Boolean),
    chinese: chinese
      .map((item) => sanitizeUnsupportedEvidenceText(item, claims))
...
export function sanitizeCreationListingUnsupportedEvidenceTerms(value, sourceFacts = {}) {
  ...
  const zhDisplay = value.zhDisplay && typeof value.zhDisplay === "object"
    ? { ...value.zhDisplay }
    : value.zhDisplay;
  for (const field of ["title", "description", "backendSearchTerms"]) {
    sanitized[field] = sanitizeUnsupportedEvidenceText(value[field], claims);
    if (zhDisplay) zhDisplay[field] = sanitizeUnsupportedEvidenceText(zhDisplay[field], claims);
  }
  for (const field of ["sellingPoints", "painPoints", "fiveBullets"]) {
    const pair = sanitizeUnsupportedEvidenceArrayPair(value[field], zhDisplay?.[field], claims);
  ...
  for (const key of bucketKeys) {
    const pair = sanitizeUnsupportedEvidenceArrayPair(englishBuckets[key], chineseBuckets[key], claims);
