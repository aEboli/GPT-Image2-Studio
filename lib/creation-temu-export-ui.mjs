...
export function createCreationTemuExportController({
  ...
  fetchImpl = fetch,
  urlApi = URL,
  // 注入后，点击「导出 Temu Excel」改为打开上品工作台覆盖层；
  ...
  // 三条守卫单独抽出，供 open 与 openExportDialog 共用：
  // 无论去向是工作台还是批量导出对话框，拦截条件与提示文案都必须一致。
  function passesOpenGuards() {
    if (isBusy() || startBlocked || isMutationBusy?.()) {
      setRecordFeedback?.("当前记录正在生成、刷新或删除，请完成后再打开 Temu 导出。", "error");
      return false;
    }
    if (getCheckedSets().length === 0) {
  ...
  // 点击「导出 Temu Excel」的去向。注入 openWorkbench 时打开上品工作台覆盖层；
  // 未注入时退回原行为，使本模块单独使用仍自洽。
  function open() {
    if (!passesOpenGuards()) return;
    if (typeof openWorkbench === "function") {
      openWorkbench(getCheckedSetIds());
      return;
    }
    openExportDialog();
  }
