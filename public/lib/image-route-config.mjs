...
export function normalizeImageRoute(value) {
  ...
  if (normalized === IMAGE_ROUTE_D || normalized === "route-d" || normalized === "grok") {
    return IMAGE_ROUTE_D;
  }
...
export function normalizeGrokEndpointPath(value, fallback = API_ENDPOINT_IMAGE_GENERATIONS) {
  const normalized = normalizeApiEndpointPath(value, fallback);
  return normalized === API_ENDPOINT_IMAGE_EDITS || normalized === API_ENDPOINT_IMAGE_GENERATIONS
    ? normalized
    : API_ENDPOINT_IMAGE_GENERATIONS;
}
...
export function normalizeImageRouteConfig(
  ...
  const routeD = source.routeD && typeof source.routeD === "object" ? source.routeD : {};
  ...
  const grokBaseInput = firstString(
    [routeD.baseUrl, source.grokBaseUrl],
    DEFAULT_GROK_BASE_URL,
  );
  const grokEndpointFallback = normalizeGrokEndpointPath(
    firstString([routeD.endpointPath, source.grokEndpointPath], API_ENDPOINT_IMAGE_GENERATIONS),
    API_ENDPOINT_IMAGE_GENERATIONS,
  );
  const grokEndpoint = splitApiEndpointUrl(grokBaseInput, {
    fallbackBaseUrl: DEFAULT_GROK_BASE_URL,
    fallbackEndpointPath: grokEndpointFallback,
  });
  ...
  const grokImageModel = firstString(
    [routeD.imageModel, source.grokImageModel],
    DEFAULT_GROK_IMAGE_MODEL,
  );
  ...
  return {
    ...
    grokBaseUrl:
      preserveRootBaseUrl(
        grokBaseInput,
        grokEndpoint.baseUrl,
        preserveRootBaseUrls,
        "grokBaseUrl",
      ) || DEFAULT_GROK_BASE_URL,
    grokEndpointPath: normalizeGrokEndpointPath(grokEndpoint.endpointPath, grokEndpointFallback),
    grokApiKey: firstString([routeD.apiKey, source.grokApiKey]),
    grokImageModel: grokImageModel || DEFAULT_GROK_IMAGE_MODEL,
  };
}
...
export function getSelectedImageGenerationConfig(config = {}) {
  ...
  if (normalized.imageRoute === IMAGE_ROUTE_D) {
    return {
      imageRoute: IMAGE_ROUTE_D,
      baseUrl: normalized.grokBaseUrl,
      endpointPath: normalized.grokEndpointPath,
      apiKey: normalized.grokApiKey,
      responsesModel: normalized.grokImageModel,
      imageModel: normalized.grokImageModel,
    };
  }
