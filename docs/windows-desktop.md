...
`GPT-Image2-Studio-Desktop-Setup-v0.2.033-x64.exe` 是使用 Electron 和 NSIS 构建的 Windows x64 桌面安装包。安装后会以独立应用窗口运行完整工作台，不需要单独安装 Node.js，也不会用浏览器标签页承载主界面。
...
| 产物 | 启动界面 | 运行时 | 适用场景 |
| --- | --- | --- | --- |
| `GPT-Image2-Studio-Desktop-Setup-v0.2.033-x64.exe` | 独立桌面窗口 | Electron 内置 Node.js | 推荐给日常 Windows 桌面用户 |
| `GPT-Image2-Studio-Portable-v0.2.033-x64.zip` | 解压后独立桌面窗口 | Electron 内置 Node.js | 不写入安装记录的便携运行 |
| `GPT-Image2-Studio-Setup-v0.2.033.exe` | 默认浏览器标签页 | 独立 `runtime\node.exe` | 保留兼容的旧启动方式 |
