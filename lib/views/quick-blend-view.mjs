...
export function createQuickBlendController(options = {}) {
  ...
  const quickBlendFilmstripRevealTracker = createFilmstripRevealTracker();
...
function renderQuickBlendGenerationPreview() {
  ...
  function renderQuickBlendGenerationStrip() {
    renderFilmstripPreservingSelection({
      strip: refs.quickBlendGenerationStrip,
      selectedKey: state.quickBlend.previewKey,
      getEntryKey: (entry) => entry.dataset?.quickBlendGenerationEntryKey || "",
      tracker: quickBlendFilmstripRevealTracker,
      render: renderQuickBlendGenerationStripEntries,
    });
  }
