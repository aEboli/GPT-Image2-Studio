...
export function createImageEditController(options = {}) {
  ...
  function syncImageEditSize(value) {
    ...
    refs.imageEditSizeInput.value = normalizeSizeForSelectedRoute(ratioValue, value);
  }
  ...
  function createImageEditJob(payload = null) {
    ...
    const sizeSetting = normalizeSizeForSelectedRoute(ratioValue, refs.imageEditSizeInput.value);
    ...
    return {
      ...
      baseUrl: generationConfig.baseUrl || state.config?.baseUrl || refs.baseUrlInput?.value?.trim() || "",
      responsesModel: generationConfig.responsesModel || state.config?.responsesModel || refs.responsesModelInput?.value?.trim() || DEFAULT_RESPONSES_MODEL,
  ...
  function setImageEditGenerationPlaceholderText(message, hidden = false) {
    stopGenerationLoadingShell(imageEditLoadingShellNodes?.loading);
  ...
  function renderImageEditGenerationLoading(item) {
    ...
    if (
      refs.imageEditGenerationPlaceholder.firstChild !== imageEditLoadingShellNodes.shell ||
      refs.imageEditGenerationPlaceholder.childElementCount !== 1
    ) {
      refs.imageEditGenerationPlaceholder.replaceChildren(imageEditLoadingShellNodes.shell);
    }
  }
