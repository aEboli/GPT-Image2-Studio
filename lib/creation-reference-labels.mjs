...
const ITEM_REFERENCE_ROLE_PRIORITIES = {
  hero: [],
  benefit: ["feature", "material"],
  scene: ["scene"],
  "multi-angle": ["material"],
  atmosphere: ["scene"],
  "product-detail": ["feature", "material"],
  "brand-story": ["scene", "feature", "material"],
  "size-capacity-fit": ["dimensions"],
  "effect-comparison": ["feature", "material", "dimensions"],
  ...
  "after-sales": ["usage", "feature", "material"],
  "usage-suggestion": ["feature", "usage", "scene"],
  "human-handheld": ["scene", "usage"],
  "human-wearable": ["scene", "dimensions"],
  sku: ["dimensions", "feature", "material", "usage", "scene"],
};
...
function getReferenceRoleIdentityInstruction(role = null, { primaryAnchor = false } = {}) {
  ...
  if (isCreationSubjectReferenceRole(roleValue) || roleLabel.includes("product subject") || roleLabel.includes("reference subject")) {
    return primaryAnchor
      ...
      : "Supporting product reference: use this image for its assigned comparison, view, or variant evidence while the primary product identity stays fixed.";
  }
  return "Supporting reference: use this image for its assigned constraint while the primary product identity stays fixed.";
}
...
function getPrimaryProductReferenceImage(images = [], referenceImageRoles = []) {
  const productReference = images.find((image, index) => getReferenceRoleValue(referenceImageRoles, image, index) === CREATION_REFERENCE_PRODUCT_ROLE);
  const subjectReference = images.find((image, index) => isCreationSubjectReferenceRole(getReferenceRoleValue(referenceImageRoles, image, index)));
  if (productReference || subjectReference) return productReference || subjectReference;
  const hasExplicitRoles = Array.isArray(referenceImageRoles) && referenceImageRoles.some((entry) => cleanString(entry?.role));
  return hasExplicitRoles ? null : images[0] || null;
}

function getPrimaryReferenceImageIndex(images = [], referenceImageRoles = []) {
  const productIndex = images.findIndex((image, index) =>
    getReferenceRoleValue(referenceImageRoles, image, index) === CREATION_REFERENCE_PRODUCT_ROLE,
  );
  if (productIndex >= 0) {
    return productIndex;
  }
  ...
  return images.findIndex((image, index) =>
    getReferenceRoleValue(referenceImageRoles, image, index) === "product",
  );
}
...
function selectSmartReferenceImages({
  ...
  const anchorIndex = requestedAnchorIndex >= 0 ? requestedAnchorIndex : -1;
  const selectedAnchor = anchorIndex >= 0 ? normalizedImages[anchorIndex] : null;
  ...
  const supporting = normalizedImages.filter((image, index) => selectedIndexes.has(index) && index !== anchorIndex);
  return selectedAnchor ? [selectedAnchor, ...supporting] : supporting;
}
...
export function buildCreationReferenceImageLabels(referenceImages = [], referenceImageRoles = []) {
  ...
  const primaryIndex = getPrimaryReferenceImageIndex(images, referenceImageRoles);
  ...
  return names.map((name, index) => {
    ...
    const roleLabel = cleanString(role?.rolePromptLabel || role?.promptLabel || role?.roleLabel || role?.role || "supporting reference");
    const instruction = cleanString(role?.promptInstruction || role?.instruction)
      .replace(/[.!?。！？]+$/u, "")
      .slice(0, 120);
    const note = cleanString(role?.note || role?.analysisNote || role?.description)
    ...
    const identityInstruction = getReferenceRoleIdentityInstruction(role, { primaryAnchor: isPrimary });
    const roleSummary = role
      ? `Role: ${roleLabel}.${instruction ? ` ${instruction}.` : note ? ` Note: ${note}.` : ""}`
      : "";
    return [
      `Reference ${index + 1}: ${name}.`,
      roleSummary,
      identityInstruction,
      isPrimary ? "Subject anchor: preserve this product identity while changing only the requested scene and layout." : "",
    ]
      .filter(Boolean)
      .join(" ");
  });
