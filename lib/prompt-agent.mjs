...
const CREATION_REFERENCE_ANALYSIS_PRODUCT_NAME_INSTRUCTION =
  "Product naming: always return product_name. Identify the specific sellable product name from the reference set. If a brand or model is visible, include it; if not, return a concise generic ecommerce product name such as 'jointed fishing lure' rather than a category path. Do not leave product_name empty.";
...
export const CREATION_REFERENCE_ANALYSIS_JSON_SCHEMA = {
  ...
  properties: {
    ...
    product_name: {
      type: "string",
      minLength: 1,
      description: "The identified sellable product name to fill the Creation Mode product name field. Use a concise product name, not a category path; include visible brand or model text only when it is actually readable.",
    },
    category_hint: {
      type: "string",
