...
function getPackageFilename(response) {
  ...
  return disposition.match(/filename="?([^";]+)"?/i)?.[1] ||
    "GPT-Image2-Studio-Product-Image-Collector-v1.1.17.zip";
}
...
export function createProductImageImportController({
  ...
  const state = { busy: false, manifest: null, previewTrigger: null, selectedIds: new Set(), toastTimer: 0 };
  ...
  function render() {
    ...
    for (const category of ["main", "detail", "sku"]) {
      ...
      for (const item of items) {
        ...
        image.addEventListener("error", () => preview.classList.add("is-error"));
        const copy = createElement(documentRef, "span", "product-image-import-preview-copy");
        copy.appendChild(createElement(documentRef, "strong", "", `${CATEGORY_LABELS[category]} ${item.order}`));
