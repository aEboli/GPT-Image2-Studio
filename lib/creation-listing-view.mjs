...
export function createCreationListingController(context = {}) {
  ...
  async function generate(setId = "") {
    ...
    try {
      ...
      const generatedDrafts = Array.isArray(payload.set?.listingDrafts)
        ? payload.set.listingDrafts
        : Array.isArray(payload.listingDrafts) ? payload.listingDrafts : [];
      const hasCompletePhysicalFields = generatedDrafts.length > 0 && generatedDrafts.every((draft) => {
        const zhDisplay = draft?.zhDisplay && typeof draft.zhDisplay === "object" ? draft.zhDisplay : {};
        return [
          draft?.packageDimensions ?? draft?.package_dimensions,
          draft?.productDimensions ?? draft?.product_dimensions,
          zhDisplay.packageDimensions ?? zhDisplay.package_dimensions,
          zhDisplay.productDimensions ?? zhDisplay.product_dimensions,
          draft?.packageWeight ?? draft?.package_weight,
          draft?.productWeight ?? draft?.product_weight,
          zhDisplay.packageWeight ?? zhDisplay.package_weight,
          zhDisplay.productWeight ?? zhDisplay.product_weight,
        ].every((value) => cleanCreationListingText(value));
      });
      if (!hasCompletePhysicalFields) {
        throw new Error("Listing 生成结果缺少包装尺寸或产品尺寸，或缺少包装重量或产品重量。当前服务可能仍是旧版本，请重启 Image Studio 后重新生成。");
      }
