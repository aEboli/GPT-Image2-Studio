...
export const CREATION_IMAGE_COUNT_OPTIONS = [4, 6, 8, 10, 12, 14, 16];
const DEFAULT_CREATION_IMAGE_COUNT = 16;
...
export const CREATION_ITEM_ROLES = [
  {
    ...
    title: "首屏主视觉",
    filenameToken: "hero",
    brief: "first-screen hero visual with the product centered as the main subject and a clear main-title plus subtitle value proposition",
  },
  {
    role: "benefit",
    title: "核心卖点图",
    filenameToken: "benefit",
    brief: "core selling-point image with the product centered and three key advantage labels or badges pointing to matching product areas",
  },
  {
    role: "scene",
    title: "使用场景图",
    filenameToken: "scene",
    brief: "real-life usage scene image with the product naturally integrated and a main-title plus subtitle that creates practical use atmosphere",
  },
  {
    role: "multi-angle",
    title: "多角度图",
    filenameToken: "angles",
    brief: "multi-angle product display with 3-4 clean-background product views and no text",
  },
  {
    role: "atmosphere",
    title: "场景氛围图",
    filenameToken: "mood",
    brief: "lifestyle atmosphere image that presents the product inside an emotional environment with a polished lifestyle rendering style",
  },
  {
    role: "product-detail",
    title: "商品细节图",
    filenameToken: "detail",
    brief: "product detail image with local close-ups or macro views and callouts pointing to concrete fine details",
  },
  {
    role: "brand-story",
    title: "品牌故事图",
    filenameToken: "brand",
    brief: "brand story image led by brand idea copy, combining the product with brand name and positioning",
  },
  {
    role: "size-capacity-fit",
    title: "尺寸/容量/尺码图",
    filenameToken: "size",
    brief: "dimension, capacity, or size-reference image with the product and a reference object, callout lines, and data labels showing physical scale",
  },
  {
    role: "effect-comparison",
    title: "效果对比图",
    filenameToken: "compare",
    brief: "before-after or split-screen comparison image using different background colors to separate the two sides clearly",
  },
  {
    role: "spec-table",
    title: "详细规格/参数表",
    filenameToken: "specs",
    brief: "structured specification or parameter table image with a strong solid background and black parameter rows or text",
  },
  {
    role: "craft-process",
    title: "工艺制作图",
    filenameToken: "craft",
    brief: "craft or production-process image showing manufacturing steps and annotated key process stages",
  },
  {
    role: "accessory-gift",
    title: "配件/赠品图",
    filenameToken: "accessories",
    brief: "accessory and gift image showing all bundled accessories laid out flat with every accessory name clearly labeled",
  },
  {
    role: "series-showcase",
    title: "系列展示图",
    filenameToken: "series",
    brief: "series display image arranging multiple colors or SKUs and labeling each style name or color number",
  },
  {
    role: "ingredient-material",
    title: "商品成分图",
    filenameToken: "ingredients",
    brief: "product ingredient or material composition visualization with icons and text explaining components or materials",
  },
  {
    role: "after-sales",
    title: "售后保障图",
    filenameToken: "service",
    brief: "after-sales guarantee image that iconifies warranty, quality assurance, refund, exchange, or service policies with concise copy",
  },
  {
    role: "usage-suggestion",
    title: "使用建议图",
    filenameToken: "tips",
    brief: "usage recommendation image with numbered step diagrams and concise explanatory text",
  },
];
...
export const CREATION_SCENARIO_ROLE_INSTRUCTIONS = {
  ...
  "social-seeding": {
    ...
    "usage-suggestion":
      "Role focus: keep advice lightweight and feed-native so it feels useful instead of like a hard-sell ad.",
  },
  launch: {
    default:
...
const CREATION_ROLE_INTENT_INSTRUCTIONS = {
  ...
  "multi-angle":
    "Role intent: present the same exact product from 3-4 angles in a clean arrangement. Keep the background dry and uncluttered, avoid marketing text, and preserve product shape, colors, markings, and proportions across every angle.",
  atmosphere:
    "Role intent: make the product feel desirable in a lifestyle environment while the product remains recognizable and commercially inspectable.",
  "product-detail":
    "Role intent: use local close-up panes, macro crops, and callout labels to identify concrete visible details, surfaces, edges, structure, controls, or finish quality without inventing hidden internals.",
  "size-capacity-fit":
  ...
  "effect-comparison":
    "Role intent: create a clear before-after, left-right, or top-bottom comparison. Use separate color backgrounds or panels so the viewer instantly understands the difference.",
  "spec-table":
  ...
  "craft-process":
    "Role intent: show production, craft, assembly, material processing, or quality-control steps in a staged process sequence, with concise labels for key steps.",
  "accessory-gift":
  ...
  "series-showcase":
    "Role intent: arrange available colors, styles, bundles, or SKU variants as one coherent series. Label each variant with a short style name, color name, or SKU marker only when supplied or safely inferable.",
  "ingredient-material":
    "Role intent: visualize supplied ingredients, materials, components, or composition facts with simple icons, material swatches, and short explanatory labels. Do not invent ingredients, certifications, or performance claims.",
  "after-sales":
    "Role intent: turn supplied after-sales, warranty, refund, exchange, shipping, quality assurance, or service policy into a clean icon-led reassurance image. Do not invent policy durations or guarantees.",
  "usage-suggestion":
...
const CREATION_ROLE_RENDERING_CONSTRAINTS = {
  ...
  "after-sales":
    "For this after-sales image, avoid fake seals, fake certificates, fake legal badges, unsupported policy durations, or guarantees that were not supplied.",
};
...
const CREATION_ROLE_CONTENT_CATEGORIES = {
  ...
  "multi-angle": ["identity", "material"],
  atmosphere: ["scene", "benefit", "visible-copy"],
  ...
  "after-sales": ["trust", "usage", "visible-copy"],
...
const CREATION_ART_DIRECTED_ROLES = new Set(["benefit", "scene", "atmosphere", "usage-suggestion", "brand-story", "after-sales"]);
...
function buildCreationSuiteSplitGuidance(role, slotIndex, totalCount) {
  const base =
    "Suite split guidance: in a 16-image suite, do not turn every image into a dense information board; distribute core facts across a small set of information images and use the remaining images for desire, scenario imagination, trust, and purchase momentum.";
...
function buildCreationUseSceneOpportunityInstruction(role, allocation, referenceImageRoles = []) {
  ...
  if (role === "usage-suggestion") {
    return `Usage opportunity: MUST include at least one dedicated charging/connection panel when the product has charging cues; build it as a charging step sequence with numbered steps, cable/port orientation, safe connection, charging duration when supplied, and ready-to-use state. Source usage cues: ${chargingCue}.`;
  }
  if (role === "atmosphere") {
