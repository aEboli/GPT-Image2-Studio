...
`GPT-Image2-Studio-Setup-v0.2.12.exe` 是 Windows 自解压安装包，由系统自带 `iexpress.exe` 生成。由于 IExpress 构建持续报错，`v0.2.12` GitHub Release 不附带这个兼容包；当前发布请使用 [Windows 桌面程序](./windows-desktop.md) 的 NSIS 安装包或免安装 ZIP。
