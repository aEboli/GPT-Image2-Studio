...
// The template carries three distinct styles across its 51 data-start cells (A2-W2 centered,
// X2-Z2 unstyled, AA2-AY2 wrap-only), so every written row copies its own column's style instead
// of a single shared constant. Capturing happens before the first write because writing row 2
// would otherwise leak its "@" number format into the style later rows read.
function captureDataRowStyles(templateSheet) {
  return TEMU_TEMPLATE_HEADERS.map((_, columnIndex) => {
    const templateStyle = templateSheet.getCell(TEMPLATE_DATA_START_ROW, columnIndex + 1).style ?? {};
    // Two independent clones so the text variant's number format can never reach the plain variant.
    const plain = structuredClone(templateStyle);
    const text = structuredClone(templateStyle);
    text.numFmt = "@";
    return { plain, text };
  });
}
...
function writeDataRows(templateSheet, plan, issues) {
  ...
  const columnStyles = captureDataRowStyles(templateSheet);

  plan.rows.forEach((row, rowIndex) => {
    const targetRow = rowIndex + TEMPLATE_DATA_START_ROW;
    ...
    TEMU_TEMPLATE_HEADERS.forEach((header, columnIndex) => {
      ...
      if (rawValue === null || rawValue === undefined || rawValue === "") {
        cell.style = columnStyles[columnIndex].plain;
        cell.value = null;
        return;
      }
      ...
      // The number format travels inside the assigned style so no cell ever mutates a style object
      // that other cells of the same column share.
      cell.style = columnStyles[columnIndex].text;
...
async function assertGeneratedWorkbook(buffer, { rowCount, issueSheetName, issueCount }, templateSheetCount) {
  const workbook = new ExcelJS.Workbook();
  await workbook.xlsx.load(buffer);
  const { templateSheet } = assertTemplateStructure(workbook);
  if (issueSheetName === null) {
    if (workbook.worksheets.length !== templateSheetCount) {
      throw workbookError("Temu 导出未预期地新增了工作表。");
    }
  } else {
...
function resolveMaxOutputBytes(maxOutputBytes) {
  if (maxOutputBytes === null || maxOutputBytes === undefined) return null;
  if (typeof maxOutputBytes !== "number" || !Number.isFinite(maxOutputBytes) || maxOutputBytes <= 0) {
    throw workbookError("Temu 导出体积上限必须是正的有限字节数。");
  }
...
export async function buildTemuWorkbookBuffer({
  ...
  includeIssueSheet = true,
  maxOutputBytes = null,
} = {}) {
  // No default size cap: the batch path can legitimately emit multi-megabyte workbooks because its
  // issue sheet is unbounded (rows x missing required fields). Only the workbench path passes a cap.
  const sizeLimit = resolveMaxOutputBytes(maxOutputBytes);
  const { workbook, templateSheet } = await loadVerifiedTemplate({ templatePath, expectedSha256 });
  const templateSheetCount = workbook.worksheets.length;
  const issues = Array.isArray(plan?.issues) ? plan.issues.map((issue) => ({ ...issue })) : [];
  writeDataRows(templateSheet, plan, issues);
  const name = includeIssueSheet ? writeIssueSheet(workbook, issues) : null;
  const buffer = Buffer.from(await workbook.xlsx.writeBuffer());
  if (sizeLimit !== null && buffer.length > sizeLimit) {
    const error = new Error(`Temu 导出工作簿为 ${buffer.length} 字节，超过 ${sizeLimit} 字节上限。`);
    error.code = "TEMU_WORKBOOK_TOO_LARGE";
    throw error;
  }
  const result = {
    buffer,
    ...
    // Skipping the issue sheet would otherwise discard the writer's own cell-change issues
    // (formula literals, 32767 truncation), so every caller can still report them.
    issues,
  };
  await assertGeneratedWorkbook(buffer, result, templateSheetCount);
  return result;
}
