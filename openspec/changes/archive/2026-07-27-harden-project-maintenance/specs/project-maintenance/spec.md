...
The repository SHALL provide a release consistency command that compares the package version with the lockfile, README, Windows distribution documentation, and current release notes. A strict release mode SHALL additionally require a clean worktree and a matching version tag on the current commit.

#### Scenario: Maintainer checks an ordinary branch
- **WHEN** the maintainer runs the standard release consistency command
- **THEN** version-bearing project files are checked for the package version
