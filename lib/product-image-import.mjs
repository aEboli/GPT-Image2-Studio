...
export function isTrustedProductSourceUrl(value) {
  const url = parseTrustedHttpsUrl(value);
  return Boolean(url && isHostOrSubdomain(url.hostname, "1688.com"));
}

export function isTrustedProductImageUrl(value) {
  const url = parseTrustedHttpsUrl(value);
  return Boolean(
    url &&
      (isHostOrSubdomain(url.hostname, "alicdn.com") || isHostOrSubdomain(url.hostname, "1688.com")),
  );
}
...
function normalizeManifestItem(rawItem, index) {
  ...
  if (!parsedUrl || !isTrustedProductImageUrl(parsedUrl.href)) {
...
export function normalizeProductImageImportManifest(rawManifest) {
  ...
  if (platform !== "1688" || !sourceUrl || !isTrustedProductSourceUrl(sourceUrl.href)) {
    throw new Error("商品图采集清单必须来自受支持的 1688 商品页。");
  }
  ...
  for (let index = 0; index < rawManifest.items.length; index += 1) {
    const item = normalizeManifestItem(rawManifest.items[index], index);
  ...
  return {
    ...
    product: {
      ...
      title: stringWithin(rawManifest.product?.title, 200) || "1688 商品",
    },
...
export function buildProductImageDownloadPlan(rawManifest, selectedIds, { clock = () => new Date() } = {}) {
  ...
  const productSegment = sanitizeProductImagePathSegment(
    [manifest.product.id, manifest.product.title].filter(Boolean).join("-") || "1688-product",
    { fallback: "1688-product", maxLength: 96 },
  );
