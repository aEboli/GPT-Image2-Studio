...
export const MAX_CREATION_ITEM_PROMPT_CHARS = 3300;
export const MAX_CREATION_INFOGRAPHIC_PROMPT_CHARS = 1200;
const MAX_CREATION_ITEM_BASE_PROMPT_CHARS = 2700;
...
function buildCreationScenarioPromptInstruction(scenario, roleValue) {
  ...
  return roleFocus
    ? `Scenario: ${normalized.label}. Role focus: ${roleFocus.replace(/^Role focus:\s*/iu, "")}`
...
function buildCreationPlatformPromptInstruction(platform, industryTemplate, roleValue, { includeStrategy = true } = {}) {
  ...
  return [
    `Platform fit: ${normalizedPlatform.label}; product category: ${categoryText}.`,
...
function compactCreationPositiveInstruction(value) {
  return cleanString(value)
    ...
    .replace(/\bwithout fake UI or unsupplied logos\b/giu, "with supplied UI and logos")
    ...
    .replace(/\band avoid fake UI or unsupplied logos\b/giu, "with supplied UI and logos")
...
function buildCreationVisualLanguageQualityLine(visualLanguage) {
  ...
  if (option.value === DEFAULT_CREATION_VISUAL_LANGUAGE) {
    return "Quality: clear composition, legible copy, realistic surfaces, and supplied-product evidence only.";
  }

  return "Quality: clear composition, legible copy, realistic surfaces, selected visual language, and supplied-product evidence only.";
}
...
function buildCreationPrimarySubjectLock(referenceImageRoles = []) {
  ...
  return `Primary subject: ${primarySubject.filename}. Preserve its silhouette, proportions, colorway, materials, logos, markings, hardware, and visible structure.`;
}
...
function buildCreationSkuPrompt({
  ...
  return buildBoundedCreationItemPrompt(
    [
    ...
    MAX_CREATION_ITEM_BASE_PROMPT_CHARS,
  );
}
...
function buildCreationTargetLanguageTextGuidance(targetLanguage) {
  ...
  return `Use concise ${targetLabel} for new canvas text outside the physical subject; preserve existing subject text, brand/model names, numbers, and units in their original form.`;
}

function buildCreationFinalTargetLanguageBoundary(targetLanguage, textPolicy = "") {
  const planningMetadataBoundary = "Planning labels for platform, scenario, category, and visual language stay internal.";
  if (cleanString(textPolicy).toLowerCase() === "none") {
    return `Canvas text uses existing subject-surface markings only. ${planningMetadataBoundary}`;
  }
  const targetLabel = cleanString(targetLanguage?.label) || "the selected target language";
...
function buildCreationPlatformItemPolicyGuidance(platformPlan = {}, slot = {}) {
  const guidance = [
    `Platform asset: ${slot.imageTypeLabel || slot.imageType}; ${slot.composition || "product-led"} composition and ${slot.scenePolicy || "product-led"} scene.`,
    slot.textPolicy === "none" ? "Use existing product markings as the visible text." : "Use concise canvas text supported by supplied evidence.",
  ];
  if (slot.logoPolicy === "forbid-overlay") {
    guidance.push("Use identifiers printed on the supplied product as branding.");
  }
...
function buildBoundedCreationItemPrompt(requiredSections = [], optionalSections = [], maxChars = MAX_CREATION_ITEM_PROMPT_CHARS) {
  const required = [...new Set(requiredSections.map(normalizeCreationPromptSection).filter(Boolean))];
  const optional = [...new Set(optionalSections.map(normalizeCreationPromptSection).filter(Boolean))];
  const join = (parts) => parts.join(" ").replace(/[ \t\f\v]+/gu, " ").replace(/\n{3,}/gu, "\n\n").trim();
  const selected = [...required, ...optional];

  for (let index = optional.length - 1; index >= 0 && join(selected).length > maxChars; index -= 1) {
    const optionalIndex = selected.lastIndexOf(optional[index]);
    if (optionalIndex >= required.length) selected.splice(optionalIndex, 1);
  }

  const compact = join(selected);
  if (compact.length <= maxChars) return compact;
  const requiredText = join(required);
  if (requiredText.length >= maxChars) {
    return `${requiredText.slice(0, Math.max(0, maxChars - 1)).trim()}…`;
  }
  const remaining = maxChars - requiredText.length - 1;
  return `${requiredText} ${join(optional).slice(0, Math.max(0, remaining)).trim()}…`;
}
...
export function buildCreationPlan(input = {}) {
  ...
  const carouselItems = plannedRoles.map((role, index) => {
    ...
    const rolePrompt = platformNativeSlot
      ? `Create ${platformPlan.platformLabel} ${platformSlot.imageTypeLabel || platformSlot.imageType} as a platform-native gallery asset. ${roleDirective}`
      : roleDirective;
    const generatedPrompt = buildBoundedCreationItemPrompt(
      [
        rolePrompt,
        `Product: ${productLine}.`,
        factsPrompt,
  ...
  const skuItems = (skuGenerationEnabled ? skuSubjects : []).map((skuSubject, index) => {
    ...
    return {
      ...
      conversionIntent,
      prompt: buildBoundedCreationItemPrompt(
        [
