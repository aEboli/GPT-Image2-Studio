...
function normalizeCreationItem(item = {}, publicBasePath) {
  ...
  for (const key of [
    "imageType", "imageTypeLabel", "ratio", "ratioLabel", "resolutionTier", "effectiveSize",
...
export function normalizeCreationSetManifest(manifest = {}, { publicBasePath = "/output" } = {}) {
  ...
  const carouselImageCount = hasExplicitFiniteNumber(manifest.carouselImageCount)
    ? Number(manifest.carouselImageCount)
    : items.filter((item) => item.role !== "sku" && item.role !== "infographic-rebuild").length;
  const skuImageCount = hasExplicitFiniteNumber(manifest.skuImageCount)
    ? Number(manifest.skuImageCount)
    : items.filter((item) => item.role === "sku").length;
  const infographicRebuildCount = hasExplicitFiniteNumber(manifest.infographicRebuildCount)
    ? Number(manifest.infographicRebuildCount)
    : items.filter((item) => item.role === "infographic-rebuild").length;
  const totalPlannedItemCount = hasExplicitFiniteNumber(manifest.totalPlannedItemCount)
    ? Number(manifest.totalPlannedItemCount)
    : carouselImageCount + skuImageCount + infographicRebuildCount;
  ...
  return {
    ...
    imageCount: hasExplicitFiniteNumber(manifest.imageCount) ? Number(manifest.imageCount) : carouselImageCount,
    carouselImageCount,
    skuImageCount,
    ...
    platformItemOverrides: Array.isArray(manifest.platformItemOverrides)
      ...
      : [],
    effectivePlan: manifest.effectivePlan && typeof manifest.effectivePlan === "object"
