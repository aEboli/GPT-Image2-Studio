...
export async function consumeResponsesSse(
  ...
  async function processChunk(chunk) {
    ...
    // actually holding an image. `response.completed` is the real terminal event and
    // stays unconditional, so a stream that ends without an image still completes and
    // lets the recovery ladder decide.
...
export async function requestImageGeneration({
  ...
  while (true) {
    ...
    let result;
    if (!response.ok) {
      const responseBody = await readResponseText(response, requestSignal);
    ...
    const { responseId, originalResponseRecoveryOnly = false, ...safeResult } = result;
