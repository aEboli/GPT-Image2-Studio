...
// Marks assets that came from saving an unfinished attempt preview rather than a
// completed generation, so the gallery can tell the two apart.
export const PARTIAL_PREVIEW_ORIGIN = "partial-attempt-preview";
...
function normalizeStoredMetadata(metadata = {}, createdAt, filename) {
  ...
  const imageRoute = String(metadata.imageRoute || metadata.generationRoute || "");
  const imageModel = String(metadata.imageModel || "");
  const ratio = String(metadata.ratio || "");
  const rawSize = String(metadata.size || "").trim();
  const actualSize = String(metadata.actualSize || metadata.generatedSize || "").trim();
  const size = rawSize.toLowerCase() === "auto"
    ? imageRoute.trim().toLowerCase() === "c"
  ...
  return {
    ...
    size,
    actualSize,
    quality: normalizeStoredImageQuality(metadata.quality, { imageRoute, imageModel }),
    format: String(metadata.format || extname(filename).replace(/^\./, "")),
    imageBackground: String(metadata.imageBackground || ""),
    ...
    previewOrigin: String(metadata.previewOrigin || ""),
    ...
    // Upstream never confirmed this image; it is the last mid-generation preview
    // promoted to a result. Persisted so the output stays traceable after a reload.
    partialImageFallback: normalizeStoredBoolean(metadata.partialImageFallback),
  };
}
...
function compactStoredMetadata(metadata = {}) {
  ...
  if (normalizeStoredBoolean(source.partialImageFallback)) {
    result.partialImageFallback = true;
  }
...
export function createTimestampedFilename(input, overrides = {}) {
  ...
  const format = String(options.format || "png")
    ...
    .replace(/^\./, "")
    .toLowerCase();
  const filenamePrefix = sanitizeFilenamePart(options.filenamePrefix).slice(0, 20);
  ...
  return `${filenamePrefix ? `${filenamePrefix}-` : ""}${formatCompactHourMinutePart(createdAt)}-${keyword}-${idTail}.${format || "png"}`;
}
  ...
  for (const imageEntry of imageEntries) {
    ...
    if (nextFilename !== imageEntry.filename) {
      ...
      await moveGalleryThumbnail({
        outputDir,
        fromRelativeImagePath: imageEntry.relativePath,
        toRelativeImagePath: nextRelativePath,
      });
...
export async function saveGeneratedAsset({
  ...
  void scheduleGalleryThumbnail({ outputDir, relativeImagePath: relativePath });
...
export async function deleteGeneratedAsset({ outputDir, indexPath, filename }) {
  ...
  if (matchedEntry) {
    await deleteGalleryThumbnail({ outputDir, relativeImagePath: matchedEntry.relativePath });
  ...
  for (const imageEntry of imageEntries) {
    ...
    try {
      ...
      items.push({
        ...
        thumbnailUrl: buildGalleryThumbnailUrl(imageEntry.relativePath, createdAt),
        createdAt,
        ...
        format: normalizedMetadata.format || imageEntry.extension.replace(/^\./, ""),
        imageBackground: normalizedMetadata.imageBackground || "",
        ...
        previewOrigin: normalizedMetadata.previewOrigin || "",
