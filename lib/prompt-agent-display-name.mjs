...
export function isLegacyPromptAgentAnalysisJson(json) {
  if (!json || typeof json !== "object" || Array.isArray(json)) {
    return false;
  }

  const hasNonOrdinaryAnalysis =
    hasMeaningfulLegacyPromptAgentValue(json.image_roles) ||
    hasMeaningfulLegacyPromptAgentValue(json.reference_roles) ||
    hasMeaningfulLegacyPromptAgentValue(json.sku_subjects) ||
    hasMeaningfulLegacyPromptAgentValue(json.relationship) ||
    hasMeaningfulLegacyPromptAgentValue(json.referenceRoles) ||
    hasMeaningfulLegacyPromptAgentValue(json.audience_strategy) ||
    (String(json.visiblePresentation || "").trim() && json.visiblePresentation !== "unclear") ||
    (String(json.heightImpression || "").trim() && json.heightImpression !== "unclear") ||
    (String(json.bodyBuild || "").trim() && json.bodyBuild !== "unclear") ||
    hasMeaningfulLegacyPromptAgentValue(json.faceVisibility) ||
    hasMeaningfulLegacyPromptAgentValue(json.distinctVisibleFeatures) ||
    hasMeaningfulLegacyPromptAgentValue(json.safety) ||
    hasMeaningfulLegacyPromptAgentValue(json.confidence);
  if (hasNonOrdinaryAnalysis) {
    return false;
  }
