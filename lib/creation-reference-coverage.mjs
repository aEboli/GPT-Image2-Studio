...
export function normalizeCreationCoverageFields(item = {}) {
  return {
    coverageSources: Array.isArray(item.coverageSources) ? item.coverageSources.map((source) => ({ index: Number(source?.index || source?.referenceIndex || source?.reference_index) || 0, filename: cleanString(source?.filename), role: cleanString(source?.role), roleLabel: cleanString(source?.roleLabel), rolePromptLabel: cleanString(source?.rolePromptLabel), note: cleanString(source?.note) })).filter((source) => source.filename) : [],
