...
function buildLegacyCreationRepairPlan(creationSet = {}) {
  ...
  const plan = {
    ...
    dimensionSpecGroups: structuredClone(
      creationSet.dimensionSpecGroups || creationSet.dimension_spec_groups || [],
    ),
...
function normalizeRepairSkuSubject(entry = {}) {
  ...
  const dimensionGroups = Array.isArray(entry.dimensionGroups)
    ? structuredClone(entry.dimensionGroups)
    : Array.isArray(entry.dimension_groups)
      ? structuredClone(entry.dimension_groups)
      : [];
