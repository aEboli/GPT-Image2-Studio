...
const TRACEABLE_CONCRETE_CLAIM_PATTERNS = [
  {
    ...
    pattern: /\b(?:compact|slim|lightweight|portable(?:\s+design)?|portability|foldable|folding|collapsible|adjustable|rechargeable|reflective|ergonomic|durable|heavy[-\s]?duty|waterproof|water[-\s]?resistant|leakproof|spillproof|non[-\s]?slip|antibacterial)\b/giu,
  },
  {
    label: "concrete product attribute",
    pattern: /(?:便携设计|便携式?)/gu,
  ...
  },
  {
    ...
    pattern: /\b(?:flip[-\s]?lid|zippered|wheeled|usb(?:-c)?\s+charging|thermal\s+imaging|night[-\s]?vision|dual[-\s]?mode)\b/giu,
  },
  {
    label: "product construction or mode",
    pattern: /USB(?:[-\s]?C)?\s*充电/giu,
  },
  {
...
const CLAIM_RISK_RULES = [
  ...
  {
    id: "qualification-sensitive-performance",
    label: "qualification-sensitive performance claim",
    support: "never",
    patterns: {
      en: [
        /\b(?:high[-\s]?power(?:ed)?|high[-\s]?brightness|highly\s+bright|ultra[-\s]?bright|waterproof|water[-\s]?resistant)\b/iu,
      ],
      zh: [/(?:高功率|大功率|强劲功率|高亮度?|超高亮|超亮|防水|耐水)/u],
    },
  },
  {
...
function listingPublicContentFields(value = {}) {
  return {
    title: value?.title,
    sellingPoints: value?.sellingPoints,
    buyerObjections: value?.buyerObjections,
...
export function getCreationListingHighRiskClaimErrors(value, sourceFacts = {}) {
  ...
  const publicContent = value && typeof value === "object" && !Array.isArray(value)
    ? {
      ...listingPublicContentFields(value),
      zhDisplay: listingPublicContentFields(value.zhDisplay),
    }
    : value;
...
const TRACEABLE_CLAIM_EQUIVALENT_GROUPS = [
  ["portable", "portable design", "portability", "便携", "便携式", "便携设计"],
  ["usb charging", "usb c charging", "usb充电", "usb c充电"],
];

function equivalentClaimEvidenceValues(value) {
  const normalized = normalizeClaimEvidenceText(value);
  if (!normalized) return [];
  return TRACEABLE_CLAIM_EQUIVALENT_GROUPS.find((group) => group.includes(normalized)) || [normalized];
}

function claimHasExactEvidence(claim, evidenceValues) {
  const equivalentClaims = equivalentClaimEvidenceValues(claim);
  if (equivalentClaims.length === 0) return false;
  return evidenceValues.some((value) => {
    const normalizedEvidence = normalizeClaimEvidenceText(value);
    return equivalentClaims.some((candidate) => (
      normalizedEvidence === candidate || normalizedEvidence.includes(candidate)
    ));
  });
}

export function hasCreationListingTraceableFact(value, sourceFacts = {}) {
  return claimHasExactEvidence(value, traceableListingFactValues(sourceFacts));
}
...
function unsupportedCreationListingEvidenceClaims(value, sourceFacts = {}) {
  ...
  for (const text of listingClaimValues(value)) {
...
function sanitizeUnsupportedEvidenceText(value, claims = []) {
  ...
  for (const { claim } of claims) {
    ...
    const boundedPattern = CJK_TEXT_PATTERN.test(claim)
      ? claimPattern
      : `(?<![\\p{L}\\p{N}])${claimPattern}(?![\\p{L}\\p{N}])`;
    text = text.replace(new RegExp(boundedPattern, "giu"), " ");
  }
...
function sanitizeLegacyListingContent(value, sanitizeText) {
  ...
  for (const field of ["buyerObjections", "highlights", "searchTerms"]) {
    if (!Object.hasOwn(value, field) && !Object.hasOwn(zhDisplay || {}, field)) continue;
    const pair = sanitizeLegacyListingArrayPair(value[field], zhDisplay?.[field], sanitizeText);
    sanitized[field] = pair.english;
    if (zhDisplay) zhDisplay[field] = pair.chinese;
  }
...
function listingClaimValues(value = {}) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
