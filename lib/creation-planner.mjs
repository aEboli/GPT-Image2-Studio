...
import { buildCreationInfographicRebuildPrompt } from "./creation-generation-parameters.mjs";
import {
...
function normalizeCreationSkuColorLabels(value, splitScalar = false) {
  const isStructured = Array.isArray(value);
  const labels = Array.isArray(value)
    ? value
    : splitScalar
      ? splitCreationSkuExplicitColorNames(value)
      : cleanString(value)
        ? [value]
        : [];
...
const CREATION_SKU_COLOR_NAME_OPTIONS = [
  ...
  { zh: "深蓝色", en: "navy blue", tokens: ["深蓝色", "深蓝", "藏蓝色", "藏蓝", "海军蓝", "navy blue", "navy", "dark blue", "deep blue"] },
  { zh: "天蓝色", en: "sky blue", tokens: ["天蓝色", "天蓝", "浅蓝色", "浅蓝", "sky blue", "light blue"] },
  { zh: "亮蓝色", en: "cyan blue", tokens: ["亮蓝色", "亮蓝", "青蓝色", "青蓝", "湖蓝色", "湖蓝", "cyan blue", "cyan", "bright blue", "aqua blue", "aqua", "turquoise blue", "turquoise"] },
...
function buildCreationSkuColorNameInstruction({ skuSubject, targetLanguage } = {}) {
  ...
  return [
    ...
    subjectUnitCount > 1
      ...
      : "Add one short characteristic-color label directly below the product subject, centered under the subject and separated from any source-card overlays.",
    ...
    "Keep all supplied colors and component qualifiers together in one label for each complete product unit.",
    "Do not drop shared neutral component colors such as a black strap or gray frame when they are supplied as visible parts of the physical product.",
    groupedColorLabels
      ? `Visible SKU color labels for the grouped subject: ${groupedColorLabels}; preserve every complete label and place it below the corresponding visible product unit.`
      : colorName
      ? `Visible SKU color label under the subject: "${colorName}"; preserve every supplied color and component qualifier in one visible label below the product subject.`
