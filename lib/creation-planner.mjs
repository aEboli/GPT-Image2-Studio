...
export function normalizeCreationReferenceAnalysis(value = {}, filenames = []) {
  ...
  const audienceStrategy = normalizeCreationAudienceStrategy(source.audienceStrategy || source.audience_strategy);
...
function parseCreationEffectivePlanSnapshot(value) {
  ...
  const items = sourceItems.map((rawItem, index) => {
    ...
    if (imageTypeDefinition) item.constraints = structuredClone(imageTypeDefinition.constraints || []);
    else item.constraints = Array.isArray(item.constraints) ? structuredClone(item.constraints) : [];
    return item;
  });
...
export function buildCreationSubmittedPlan(input = {}) {
  const snapshot = parseCreationEffectivePlanSnapshot(input.effectivePlan || input.effective_plan);
...
export function buildCreationPlan(input = {}) {
  ...
  const carouselItems = plannedRoles.map((role, index) => {
    ...
    return {
      ...
      itemKind: "carousel",
      role: role.role,
      title: platformSlot.imageTypeLabel || role.title,
  ...
  const editableSlots = legacyPlanningMode
    ...
    : platformPlan.slots.map((slot) => carouselItemsBySlotKey.get(slot.slotKey) || {
      ...
      itemKind: "carousel",
      title: slot.imageTypeLabel || getCreationPlatformRoleDefinition(slot.role).title,
      prompt: slot.prompt || "",
    });
