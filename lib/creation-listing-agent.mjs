...
const PLATFORM_V1_LISTING_GUIDELINES = [
  ...
  "2. No-brand title formula: use the high-search core product keyword followed only by supplied objective attributes such as quantity, color, material, shape, model, and variant in the order preferred by the resolved platform policy.",
  ...LISTING_SEO_AGENT_GUIDELINES.slice(2),
];
  ...
  if (requestContext.legacyContract && requestSource.forceV1 === true) {
    ...
    try {
      ...
      return isCompletedLegacyDraftStructurallyUsable(draft)
        && !containsCreationListingFunctionalWording(JSON.stringify(draft))
        ? draft
