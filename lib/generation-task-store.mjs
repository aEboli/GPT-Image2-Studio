import { normalizeGenerationSize, normalizeModelProtocolImageSize } from "./generation-size-options.mjs";
...
function normalizeTaskSize(value, ratio, imageRoute) {
  const raw = String(value || "").trim();
  if (!raw) {
    return "";
  }
  if (String(imageRoute || "").trim().toLowerCase() === "c") {
    return normalizeModelProtocolImageSize(raw);
  }
  return normalizeGenerationSize(ratio || "4:5", raw);
}
...
function normalizeTask(task = {}, fallback = {}) {
  ...
  const imageRoute = task.imageRoute ?? task.generationRoute ?? fallback.imageRoute ?? fallback.generationRoute;
  const imageModel = task.imageModel ?? fallback.imageModel ?? "gpt-image-2";
  ...
  return {
    ...
    size: normalizeTaskSize(task.size ?? fallback.size, task.ratio ?? fallback.ratio, imageRoute),
    quality: normalizeStoredImageQuality(task.quality ?? fallback.quality, { imageRoute, imageModel }),
