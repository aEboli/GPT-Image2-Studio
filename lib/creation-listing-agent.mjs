...
import {
  ...
  hasCreationListingTraceableFact,
...
const LISTING_ORDINARY_ATTRIBUTE_GUIDANCE = "Ordinary supplied attributes such as portable design and USB charging are allowed and should remain available for buyer-facing copy and search terms. Do not remove them merely because they describe portability or charging.";
const LISTING_FUNCTIONAL_WORDING_GUARDRAILS = [
  ...
  LISTING_ORDINARY_ATTRIBUTE_GUIDANCE,
  "Always omit qualification-sensitive performance wording such as high-power, high-brightness, ultra-bright, waterproof, water-resistant, 高功率, 高亮, and 防水. Ordinary product text is not qualification evidence.",
];
...
const LISTING_DIMENSION_FIELD_GUIDELINES = [
  ...
  "For packageDimensions, always return length x width x height. When package evidence supplies one or two measured axes, retain those measured axes and infer only the missing axes from the product material, form, and supplied size comparison; do not label that completed measured package size as Estimated. Never invent a replacement for a supplied measured axis.",
  "When package source evidence is absent, provide a conservative numeric length x width x height estimate. Base it on the product category, material or construction, and any supplied comparable size; do not recycle a fixed generic size. Start the English value exactly with Estimated: and the Simplified Chinese value exactly with 预估：. Never present a fully estimated size as a measured or sourced fact.",
  "Every dimension value must contain at least one numeric physical length with an appropriate unit. Preserve component-specific product measurements when that is more truthful.",
  ...
  "When the corresponding weight evidence is absent, estimate it from the product category, garment or object length and construction, supplied material, sellable quantity, visible/reference context, and packaging overhead. Do not recycle one fixed generic weight across unrelated products. Keep productWeight as the estimated net item weight and packageWeight as the estimated packed/shipping weight, which must not be lower than productWeight. Start the English weight value exactly with Estimated: and the Simplified Chinese value exactly with 预估：. Follow the selected metric, imperial, or both unit mode.",
];
const PLATFORM_V1_EVIDENCE_BACKED_VALUE_GUARDRAILS = [
  ...
  LISTING_ORDINARY_ATTRIBUTE_GUIDANCE,
  "Qualification-sensitive performance boundary: always omit high-power, high-brightness, ultra-bright, waterproof, water-resistant, 高功率, 高亮, 防水, and equivalent wording. Source JSON product text is not a verified qualification document.",
...
function escapeListingRegExp(value) {
  return String(value ?? "").replace(/[.*+?^$()|[\]\\]/g, "\\$&");
}

function listingPartNumberTokens(value, { structured = false } = {}) {
  const basename = cleanString(value).split(/[\\/]/u).at(-1)?.replace(/\.[a-z0-9]{1,8}$/iu, "") || "";
  return basename
    .split(/[^\p{L}\p{N}]+/u)
    .map(cleanString)
    .filter((token) => {
      if (token.length < 3 || !/\d/u.test(token)) return false;
      if (structured) return /[\p{L}]/u.test(token) || token.length >= 4;
      return /[\p{L}]/u.test(token) && !/^\d+(?:pack|piece|pcs?)$/iu.test(token);
    });
}

function extractListingTitlePartNumbers(source = {}) {
  const tokens = new Map();
  const appendExact = (value) => {
    const token = cleanString(value).split(/[\/]/u).at(-1)?.replace(/\.[a-z0-9]{1,8}$/iu, "") || "";
    if (token.length < 3 || !/\d/u.test(token) || !/[\p{L}]/u.test(token)) return;
    const key = token.toLocaleLowerCase("en-US");
    if (!tokens.has(key)) tokens.set(key, token);
  };
  const append = (value, options) => {
    for (const token of listingPartNumberTokens(value, options)) {
      const key = token.toLocaleLowerCase("en-US");
      if (!tokens.has(key)) tokens.set(key, token);
    }
  };
  appendExact(source.skuSubjectId);
  append(source.skuSubjectId, { structured: true });
  for (const filename of Array.isArray(source.skuFilenames) ? source.skuFilenames : []) {
    appendExact(filename);
    append(filename, { structured: true });
  }
  for (const sku of Array.isArray(source.skuSubjects) ? source.skuSubjects : []) {
    appendExact(sku?.id);
    appendExact(sku?.skuId ?? sku?.sku_id);
    appendExact(sku?.partNumber ?? sku?.part_number);
    append(sku?.id, { structured: true });
    append(sku?.skuId ?? sku?.sku_id, { structured: true });
    append(sku?.partNumber ?? sku?.part_number, { structured: true });
    append(sku?.title);
    for (const filename of Array.isArray(sku?.filenames) ? sku.filenames : []) {
      appendExact(filename);
      append(filename, { structured: true });
    }
  }
  append(source.productName);
  append(source.skuTitle);
  return [...tokens.values()].sort((left, right) => right.length - left.length);
}

function sanitizeListingTitlePartNumbers(value, partNumbers = []) {
  let title = cleanString(value);
  for (const partNumber of partNumbers) {
    const pattern = escapeListingRegExp(partNumber).replace(/[-_]+/g, "[-_\\s]*");
    title = title.replace(new RegExp("(?<![\\p{L}\\p{N}])" + pattern + "(?![\\p{L}\\p{N}])", "giu"), " ");
  }
...
function sanitizeListingDraftTitlePartNumbers(draft = {}, source = {}) {
  if (!draft || typeof draft !== "object" || Array.isArray(draft)) return draft;
  const partNumbers = extractListingTitlePartNumbers(source);
  if (partNumbers.length === 0) return draft;
  const sanitized = {
    ...draft,
    title: sanitizeListingTitlePartNumbers(draft.title, partNumbers),
  };
  if (draft.zhDisplay && typeof draft.zhDisplay === "object" && !Array.isArray(draft.zhDisplay)) {
    sanitized.zhDisplay = {
      ...draft.zhDisplay,
      title: sanitizeListingTitlePartNumbers(draft.zhDisplay.title, partNumbers),
    };
  }
  return sanitized;
}

function finalizeListingDraftTitlePartNumbers(draft = {}, source = {}) {
  const sanitized = sanitizeListingDraftTitlePartNumbers(draft, source);
  const partNumbers = extractListingTitlePartNumbers(source);
  const englishWasEmptied = hasReadableListingText(draft.title) && !hasReadableListingText(sanitized.title);
  const chineseWasEmptied = hasReadableListingText(draft.zhDisplay?.title)
    && !hasReadableListingText(sanitized.zhDisplay?.title);
  if (!englishWasEmptied && !chineseWasEmptied) return sanitized;
  const repaired = { ...sanitized };
  if (englishWasEmptied) {
    repaired.title = sanitizeListingTitlePartNumbers(findSanitizedSourceIdentity(source, "en"), partNumbers) || "Product";
  }
  if (chineseWasEmptied && sanitized.zhDisplay && typeof sanitized.zhDisplay === "object") {
    repaired.zhDisplay = {
      ...sanitized.zhDisplay,
      title: sanitizeListingTitlePartNumbers(findSanitizedSourceIdentity(source, "zh"), partNumbers) || "商品",
    };
  }
  return repaired;
}

function normalizeListingTitleCompletionKey(value) {
...
const LISTING_TITLE_COMPLETION_STRUCTURAL_LABELS = new Set([
  "product type",
  "pack details",
  "package details",
  "package contents",
  ...
  "痛点",
  "尺寸",
].map(normalizeListingTitleCompletionKey));

function splitListingTitleCompletionLead(value) {
  const text = cleanString(value).replace(/^(?:[-*]+|\d+[.)、])\s*/u, "");
  const match = text.match(/^([^:：]{1,36})[:：]\s*(.*)$/u);
  if (!match) return { label: "", body: text };
  const label = cleanString(match[1]);
  return {
    label: LISTING_TITLE_COMPLETION_STRUCTURAL_LABELS.has(normalizeListingTitleCompletionKey(label)) ? "" : label,
    body: cleanString(match[2]),
  };
}

function cleanListingTitleCompletionPhrase(value, language = "en") {
  let phrase = cleanString(value)
    .replace(/^(?:[-*]+|\d+[.)、])\s*/u, "")
    .replace(/^[\s,.;:!?，。；：！？|｜_-]+|[\s,.;:!?，。；：！？|｜_-]+$/gu, "")
    .trim();
  if (!phrase) return "";
  if (language === "zh") {
    return phrase.replace(/^(?:这款|该款|该产品|本产品)\s*/u, "");
  }
  return phrase.replace(/^(?:and|or|but|while|whereas|because|although|though)\s+/iu, "")
    .replace(/^(?:this|that|these|those|the|a|an)\s+/iu, "");
}

function boundListingTitleCompletionPhrase(value, language = "en") {
  const phrase = cleanString(value);
  if (!phrase) return "";
  if (language === "zh") return Array.from(phrase).slice(0, 20).join("").trim();
  const words = phrase.split(/\s+/u).filter(Boolean).slice(0, 10);
  while (words.length > 1 && /^(?:and|or|but|for|from|in|of|on|the|to|with|without)$/iu.test(words.at(-1))) {
    words.pop();
  }
  return words.join(" ").trim();
}

function splitListingTitleCompletionCandidates(value, language = "en", source = {}) {
  const { label, body } = splitListingTitleCompletionLead(value);
  const supportedLabel = label && hasCreationListingTraceableFact(label, source) ? label : "";
  const bodyCandidates = body
    .split(/(?:[\r\n|｜]+|(?<!\d)[.!?](?!\d)|[。！？；;，,]+|\s+[–—]\s+|\s+(?:while|whereas|because|although|though)\s+)/iu);
  return [supportedLabel, ...bodyCandidates]
    .map((part) => cleanListingTitleCompletionPhrase(part, language))
    .filter(Boolean);
}

function listingTitleCompletionCandidates(draft = {}, language = "en", source = {}) {
  const display = language === "zh" && draft.zhDisplay && typeof draft.zhDisplay === "object"
    ? draft.zhDisplay
    : draft;
  const values = [
    ...(Array.isArray(display.sellingPoints) ? display.sellingPoints : []),
    ...(Array.isArray(display.fiveBullets) ? display.fiveBullets : []),
    ...(Array.isArray(display.painPoints) ? display.painPoints : []),
    display.description,
    display.backendSearchTerms,
    ...["exact", "longTail", "traffic", "descriptive"].flatMap((key) => (
      Array.isArray(display.keywordBuckets?.[key]) ? display.keywordBuckets[key] : []
    )),
  ];
  return values
    .flatMap((value) => splitListingTitleCompletionCandidates(value, language, source))
    .filter((candidate) => matchesRequiredListingLanguage(candidate, language));
}

function listingTitleCompletionPolicy(source = {}) {
  return source.listingPolicy && typeof source.listingPolicy === "object"
...
function listingTitleCompletionTarget(source = {}, language = "en") {
  const rules = listingTitleCompletionPolicy(source).titleRules || {};
  const recommendedMinChars = Number(rules.recommendedMinChars);
  let target = Number.isFinite(recommendedMinChars) ? Math.max(0, recommendedMinChars) : 35;
  const evidenceAliases = new Set((Array.isArray(source.listingEvidenceAliases) ? source.listingEvidenceAliases : [])
    .map(normalizeListingTitleCompletionKey)
    .filter(Boolean));
  const hardMaxChars = Number(rules.hardMaxChars);
  const hardMaxBytes = Number(rules.hardMaxUtf8Bytes);
  const hasLowHardLimit = (Number.isFinite(hardMaxChars) && hardMaxChars > 0 && hardMaxChars < 120)
    || (Number.isFinite(hardMaxBytes) && hardMaxBytes > 0 && hardMaxBytes < 120);
  if (language === "en" && evidenceAliases.size >= 6 && !hasLowHardLimit) target = Math.max(target, 120);
  const effectiveMaxChars = Number.isFinite(hardMaxChars) && hardMaxChars > 0
    ? hardMaxChars
    : CREATION_LISTING_FIELD_MAX_CHARS;
  return Math.min(target, effectiveMaxChars, CREATION_LISTING_FIELD_MAX_CHARS);
}

function fitListingTitleCompletionCandidate(candidate, prefix, limits = {}, language = "en") {
  const maxChars = Number.isFinite(limits.maxChars) ? limits.maxChars : CREATION_LISTING_FIELD_MAX_CHARS;
  const maxBytes = Number.isFinite(limits.maxBytes) ? limits.maxBytes : Infinity;
  const fits = (addition) => {
    const combined = `${prefix}${addition}`;
    return Array.from(combined).length <= maxChars
      && (maxBytes === Infinity || new TextEncoder().encode(combined).length <= maxBytes);
  };
  if (language === "en") {
    const words = cleanString(candidate).split(/\s+/u).filter(Boolean);
    const accepted = [];
    ...
    while (accepted.length > 1 && /^(?:and|or|but|for|from|in|of|on|the|to|with|without)$/iu.test(accepted.at(-1))) {
      accepted.pop();
    }
    return accepted.join(" ").trim();
  }
  let accepted = "";
  for (const character of cleanString(candidate)) {
    if (!fits(`${accepted}${character}`)) break;
    accepted += character;
  }
  return accepted.trim();
}

function appendListingTitleCompletionCandidate(title, candidate, limits = {}, language = "en") {
  const base = cleanString(title);
  const separator = base ? " " : "";
  const prefix = `${base}${separator}`;
  const addition = fitListingTitleCompletionCandidate(candidate, prefix, limits, language);
  return hasReadableListingText(addition) ? `${prefix}${addition}`.trim() : base;
}

function completeListingTitle(value, source = {}, draft = {}, language = "en") {
  const initial = cleanString(value);
  const target = listingTitleCompletionTarget(source, language);
  if (!initial || Array.from(initial).length >= target) return initial;
  const rules = listingTitleCompletionPolicy(source).titleRules || {};
  const hardMaxChars = Number(rules.hardMaxChars);
  const hardMaxBytes = Number(rules.hardMaxUtf8Bytes);
  const limits = {
    maxChars: Number.isFinite(hardMaxChars) && hardMaxChars > 0
      ? Math.min(hardMaxChars, CREATION_LISTING_FIELD_MAX_CHARS)
      : CREATION_LISTING_FIELD_MAX_CHARS,
    maxBytes: Number.isFinite(hardMaxBytes) && hardMaxBytes > 0 ? hardMaxBytes : Infinity,
  };
  const partNumbers = extractListingTitlePartNumbers(source);
  const titleTokens = new Set(normalizeListingTitleCompletionKey(initial).split(/\s+/u).filter(Boolean));
  const seen = new Set();
  let completed = initial;
  for (const rawCandidate of listingTitleCompletionCandidates(draft, language, source)) {
    const candidate = boundListingTitleCompletionPhrase(
      sanitizeListingTitlePartNumbers(rawCandidate, partNumbers)
        .replace(/[.!?。！？；;，,：:]+$/gu, "")
        .trim(),
      language,
    );
    const candidateKey = normalizeListingTitleCompletionKey(candidate);
    if (!candidateKey || seen.has(candidateKey)) continue;
    seen.add(candidateKey);
    const candidateTokens = candidateKey.split(/\s+/u).filter(Boolean);
    if (candidateTokens.length > 0 && candidateTokens.every((token) => titleTokens.has(token))) continue;
    const next = appendListingTitleCompletionCandidate(completed, candidate, limits, language);
    if (next === completed) continue;
    const appended = next.slice(completed.length).trim();
    completed = next;
    normalizeListingTitleCompletionKey(appended).split(/\s+/u).filter(Boolean)
      .forEach((token) => titleTokens.add(token));
    if (Array.from(completed).length >= target) break;
  }
  return completed;
}

function completeListingDraftTitles(draft = {}, source = {}) {
  if (!draft || typeof draft !== "object" || Array.isArray(draft)) return draft;
  const completed = { ...draft, title: completeListingTitle(draft.title, source, draft, "en") };
  if (draft.zhDisplay && typeof draft.zhDisplay === "object" && !Array.isArray(draft.zhDisplay)) {
    completed.zhDisplay = {
      ...draft.zhDisplay,
      title: completeListingTitle(draft.zhDisplay.title, source, draft, "zh"),
    };
  }
  return completed;
}

function listingTitlePartNumberPromptInstruction(source = {}) {
  const partNumbers = extractListingTitlePartNumbers(source);
  return [
    "Title privacy rule: title and zhDisplay.title must not include any internal product part number, SKU part number, source filename code, or reference-only identifier. These values are association metadata, not buyer-facing title attributes.",
    partNumbers.length > 0
      ? "Known internal identifiers prohibited from both titles: " + JSON.stringify(partNumbers) + "."
      : "Do not copy identifier-like values from SKU IDs or reference filenames into either title.",
  ].join(" ");
}
...
const LISTING_RESPONSE_CONTENT_FIELDS = [
  "title",
  "sellingPoints",
  "selling_points",
  "painPoints",
...
function isListingResponseObject(value) {
...
function hasNonEmptyListingResponseValue(value) {
  if (value === null || value === undefined) return false;
  if (typeof value === "string") return Boolean(cleanString(value));
  if (typeof value === "number") return Number.isFinite(value);
  if (typeof value === "boolean") return value;
  if (Array.isArray(value)) return value.some(hasNonEmptyListingResponseValue);
  if (isListingResponseObject(value)) {
    return Object.values(value).some(hasNonEmptyListingResponseValue);
  }
  return false;
}

function hasListingResponseContent(value) {
  if (!isListingResponseObject(value)) return false;
  const display = value.zhDisplay ?? value.zh_display;
  return LISTING_RESPONSE_CONTENT_FIELDS.some((field) => (
    hasNonEmptyListingResponseValue(value[field])
  )) || (
    isListingResponseObject(display)
    && LISTING_RESPONSE_CONTENT_FIELDS.some((field) => (
      hasNonEmptyListingResponseValue(display[field])
    ))
  );
}

function assertUsableParsedListingResponse(value) {
  if (!isListingResponseObject(value) || !hasListingResponseContent(value)) {
    throw new Error("Listing response did not contain a usable Listing object.");
  }
  return value;
}
  ...
  try {
    ...
    if (start >= 0 && end > start) {
      try {
        ...
        // Fall through to the stable response-parsing error below.
      }
    }
...
function collectResponseText(value, parts = []) {
  ...
  if (value.parsed && typeof value.parsed === "object") {
    parts.push(JSON.stringify(value.parsed));
  }

  if (value.output_parsed && typeof value.output_parsed === "object") {
    parts.push(JSON.stringify(value.output_parsed));
  }
  ...
  if (Array.isArray(value.choices)) {
    value.choices.forEach((item) => collectResponseText(item, parts));
  }

  if (value.message && typeof value.message === "object") {
    collectResponseText(value.message, parts);
  }
...
function parseUsableListingResponse(payload) {
  const candidates = hasListingResponseContent(payload)
    ? [JSON.stringify(payload)]
    : collectResponseText(payload);
  if (candidates.length === 0 && isListingResponseObject(payload)) {
  ...
  for (const candidate of candidates) {
    try {
      return assertUsableParsedListingResponse(parseJsonText(candidate));
    } catch (error) {
      lastError = error;
    }
  }
...
function buildLegacyListingPrompt(source = {}, responseFormatErrors = []) {
  return [
    ...
    listingTitlePartNumberPromptInstruction(source),
    ...
    responseFormatErrors.length
      ? `The previous response did not return a usable Listing JSON object. Generate the required Listing JSON object again. Response format issue: ${responseFormatErrors.join("; ")}`
      : "",
...
function buildPlatformV1ListingPrompt(source = {}, policy = {}, responseFormatErrors = []) {
  ...
  return [
    ...
    source.evidenceMode === "input-only"
      ...
      : "Generated images or saved image metadata are available. Use them for visible selling points and pain points.",
    responseFormatErrors.length
      ? `The previous response did not return a usable Listing JSON object. Generate the required structured Listing JSON again. Response format issue: ${responseFormatErrors.join("; ")}`
      : "",
    `Source JSON:\n${JSON.stringify(source, null, 2)}`,
  ].filter(Boolean).join("\n\n");
}
...
function buildPlatformListingPrompt(source = {}, policy = {}, responseFormatErrors = []) {
  ...
  return [
    ...
    `Source JSON:\n${JSON.stringify(source, null, 2)}`,
    responseFormatErrors.length
      ? `The previous response did not return a usable Listing JSON object. Generate the required structured Listing JSON again. Response format issue:\n${responseFormatErrors.join("\n")}`
      : "",
  ].filter(Boolean).join("\n\n");
}
...
function makeRequestBody({ responsesModel, reasoningEffort, source, policy, legacyContract, responseFormatErrors = [] }) {
  return {
    model: responsesModel || DEFAULT_RESPONSES_MODEL,
    reasoning: { effort: reasoningEffort || "medium" },
    input: legacyContract
      ? source.forceV1 === true
        ? buildPlatformV1ListingPrompt(source, policy, responseFormatErrors)
        : buildLegacyListingPrompt(source, responseFormatErrors)
      : buildPlatformListingPrompt(source, policy, responseFormatErrors),
    text: {
      format: {
        type: "json_schema",
...
function responseFormatErrorText(error) {
  return cleanString(error instanceof Error ? error.message : String(error))
    || "Listing response was unusable.";
}

async function requestUsableListingResponse({
  baseUrl,
  endpointPath,
  apiKey,
  responsesModel,
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    try {
      return parseUsableListingResponse(payload);
    } catch (error) {
      if (attempt === 1) throw error;
      responseFormatErrors = [responseFormatErrorText(error)];
    }
  }
...
function normalizeParsedListingDraft(parsed, source, legacyContract) {
  ...
  let sanitizedParsed = sanitizeCreationListingUnsupportedEvidenceTerms(frozenParsed, source);
  sanitizedParsed = sanitizeCreationListingBlockingClaims(sanitizedParsed, source);
  const normalizedDraft = finalizeListingDraftTitlePartNumbers({
    ...applyCreationListingPackageDimensions(
      applyCreationListingWeightFields(
        normalizeCreationListingDraft(sanitizedParsed, source),
        source,
      ),
      source,
    ),
  ...
  const originalDraftForRepair = frozenParsed.zhDisplay || frozenParsed.zh_display
    ? { ...frozenParsed, zhDisplay: frozenParsed.zhDisplay ?? frozenParsed.zh_display }
    : frozenParsed;
  return finalizeListingDraftTitlePartNumbers(
    sanitizeCompletedListingDraftClaims(normalizedDraft, source, originalDraftForRepair),
    source,
  );
}
...
function normalizeCompletedLegacyDraft(value = {}, source = {}) {
  ...
  return applyCreationListingPackageDimensions({
    ...oldStyleDraft,
...
function sanitizeCompletedListingDraftClaims(draft = {}, source = {}, originalDraft = draft) {
  let sanitized = sanitizeCreationListingUnsupportedEvidenceTerms(draft, source);
  sanitized = sanitizeCreationListingBlockingClaims(sanitized, source);
  sanitized = repairCompletedLegacyDraftRequiredText(sanitized, source, originalDraft);
  sanitized = sanitizeCreationListingUnsupportedEvidenceTerms(sanitized, source);
  return sanitizeCreationListingBlockingClaims(sanitized, source);
}
...
const CREATION_LISTING_LENGTH_UNIT_FACTORS = {
  mm: 1,
  cm: 10,
  m: 1000,
  in: 25.4,
...
function creationListingDimensionProfile(source = {}, kind = "product") {
  const text = [
    source.productName,
    source.productDescription,
    source.description,
    ...
    source.packageList,
    ...(Array.isArray(source.referenceImageRoles) ? source.referenceImageRoles.map((item) => item?.note) : []),
    ...(Array.isArray(source.skuSubjects) ? source.skuSubjects.flatMap((item) => [item?.title, item?.description, item?.note]) : []),
  ].map(cleanString).filter(Boolean).join(" ").toLowerCase();
  const packageKind = kind === "package";
  const profiles = [
    { pattern: /(?:lure|bait|minnow|fishing|路亚|鱼饵|拟饵)/u, product: [130, 40, 28], package: [160, 55, 42], widthRatio: 0.31, depthRatio: 0.28 },
    { pattern: /(?:bottle|tumbler|cup|flask|水瓶|水杯|保温杯)/u, product: [240, 75, 75], package: [270, 100, 100], widthRatio: 0.32, depthRatio: 0.32 },
    { pattern: /(?:goggle|glasses|eyewear|护目镜|眼镜|风镜)/u, product: [185, 85, 75], package: [230, 150, 105], widthRatio: 0.46, depthRatio: 0.35 },
    { pattern: /(?:tool|latch|lock|hardware|hinge|工具|插销|锁|五金|铰链)/u, product: [160, 65, 30], package: [200, 85, 45], widthRatio: 0.4, depthRatio: 0.32 },
    { pattern: /(?:electronic|charger|camera|screen|remote|battery|电子|充电器|相机|显示器|遥控|电池)/u, product: [160, 105, 45], package: [210, 145, 70], widthRatio: 0.66, depthRatio: 0.38 },
    { pattern: /(?:bag|clothing|shirt|fabric|textile|apparel|包|服装|衣|布|纺织)/u, product: [300, 220, 35], package: [330, 250, 50], widthRatio: 0.73, depthRatio: 0.16 },
    { pattern: /(?:paper|card|sticker|film|sheet|纸|卡|贴纸|薄膜)/u, product: [300, 210, 3], package: [320, 230, 15], widthRatio: 0.7, depthRatio: 0.06 },
  ];
  const profile = profiles.find((candidate) => candidate.pattern.test(text))
    || { product: [180, 110, 55], package: [210, 140, 80], widthRatio: 0.61, depthRatio: 0.38 };
  const comparisonProfiles = [
    {
      pattern: /(?:bank\s*card|credit\s*card|银行卡|信用卡|卡片大小|卡片尺寸)/u,
      product: [86, 54, 1],
      package: [110, 75, 15],
    },
    {
      pattern: /(?:smart\s*phone|cell\s*phone|mobile\s*phone|手机|手机大小|手机尺寸)/u,
      product: [150, 75, 8],
      package: [180, 100, 25],
    },
    {
      pattern: /(?:palm(?:\s*[- ]?sized)?|hand(?:\s*[- ]?sized)?|手掌(?:大小|尺寸)?|掌心大小)/u,
      product: [180, 90, 20],
      package: [210, 120, 35],
    },
    {
      pattern: /(?:coin|硬币|一元硬币|25\s*cent|quarter)/u,
      product: [24, 24, 2],
      package: [40, 40, 12],
    },
    {
      pattern: /(?:match\s*box|matchbox|火柴盒)/u,
      product: [50, 35, 15],
      package: [58, 42, 20],
    },
    {
      pattern: /(?:pen(?:cil)?|marker|笔状|笔的大小)/u,
      product: [140, 12, 12],
      package: [165, 30, 20],
    },
  ];
  const comparison = comparisonProfiles.find((candidate) => candidate.pattern.test(text));
  const base = comparison
    ? (packageKind ? comparison.package : comparison.product)
    : (packageKind ? profile.package : profile.product);
  const comparisonWidthRatio = comparison
    ? base[1] / Math.max(base[0], 1)
    : profile.widthRatio;
  const comparisonDepthRatio = comparison
    ? base[2] / Math.max(Math.min(base[0], base[1]), 1)
    : profile.depthRatio;
  const materialDepthRatio = /(?:paper|card|sticker|film|纸|卡|贴纸|薄膜)/u.test(text)
    ? 0.06
    : /(?:fabric|textile|foam|leather|cloth|布|纺织|泡棉|皮革)/u.test(text)
      ? 0.16
      : /(?:metal|steel|aluminum|iron|五金|金属|钢|铝|铁)/u.test(text)
        ? 0.27
        : profile.depthRatio;
  const thinPackaging = /(?:paper|card|sticker|film|纸|卡|贴纸|薄膜)/u.test(text);
  const softPackaging = /(?:fabric|textile|foam|leather|cloth|布|纺织|泡棉|皮革)/u.test(text);
  const accessoryPackaging = /(?:usb|cable|charger|charging|accessor|charging\s+line|充电线|数据线|线缆|配件|收纳盒|收纳箱|透明盒)/u.test(text);
  const minimumDepthFactor = thinPackaging ? 0.25 : softPackaging ? 0.45 : 0.55;
  const hasMaterialDepthSignal = /(?:paper|card|sticker|film|纸|卡|贴纸|薄膜|fabric|textile|foam|leather|cloth|布|纺织|泡棉|皮革|metal|steel|aluminum|iron|五金|金属|钢|铝|铁)/u.test(text);
  return {
    base,
    widthRatio: comparison ? comparisonWidthRatio : profile.widthRatio,
    depthRatio: comparison && !hasMaterialDepthSignal ? comparisonDepthRatio : materialDepthRatio,
    packageScale: packageKind ? (thinPackaging ? 1.08 : softPackaging ? 1.1 : 1.18) : 1,
    packageClearanceMm: packageKind ? (accessoryPackaging ? 12 : thinPackaging ? 4 : 8) : 0,
    minimumPackageWidth: packageKind ? profile.package[1] * 0.55 : 0,
    minimumPackageDepth: packageKind
      ? profile.package[2] * minimumDepthFactor * (accessoryPackaging ? 1.2 : 1)
      : 0,
  };
}

function creationListingDimensionAxisMillimeters(number, unit) {
  const normalizedUnit = normalizeCreationListingLengthUnit(unit);
  const factor = CREATION_LISTING_LENGTH_UNIT_FACTORS[normalizedUnit];
  const numeric = Number(number);
  return Number.isFinite(numeric) && factor ? numeric * factor : null;
}

function stripCreationListingDimensionConversions(value) {
  const unit = `(?:${LENGTH_UNIT_PATTERN})`;
  const measuredAxis = `\\d+(?:\\.\\d+)?\\s*${unit}`;
  const alternateUnitGroup = new RegExp(
    `(${measuredAxis})\\s*[（(]\\s*(?=[^()（）]{0,80}${unit})[^()（）]{0,80}\\s*[)）]`,
    "giu",
  );
  return cleanString(value).replace(alternateUnitGroup, "$1");
}

function firstCreationListingDimensionAxes(value) {
  const text = stripCreationListingDimensionConversions(value);
  const unit = `(${LENGTH_UNIT_PATTERN})`;
  const number = "(\\d+(?:\\.\\d+)?)";
  const threeAxes = new RegExp(
    `${number}\\s*${unit}\\s*(?:x|×|by)\\s*${number}\\s*${unit}\\s*(?:x|×|by)\\s*${number}\\s*${unit}`,
    "iu",
  );
  const twoAxes = new RegExp(
    `${number}\\s*${unit}\\s*(?:x|×|by)\\s*${number}\\s*${unit}`,
    "iu",
  );
  const sharedAxes = new RegExp(
    `${number}\\s*(?:x|×|by)\\s*${number}(?:\\s*(?:x|×|by)\\s*${number})?\\s*${unit}`,
    "iu",
  );
  const parse = (numbers, units) => {
    const axes = numbers.map((value, index) => creationListingDimensionAxisMillimeters(value, units[index] || units.at(-1)));
    return axes.every((axis) => axis !== null) ? axes : [];
  };
  const triple = text.match(threeAxes);
  if (triple) return parse([triple[1], triple[3], triple[5]], [triple[2], triple[4], triple[6]]);
  const pair = text.match(twoAxes);
  if (pair) return parse([pair[1], pair[3]], [pair[2], pair[4]]);
  const shared = text.match(sharedAxes);
  if (shared) return parse(
    [shared[1], shared[2], shared[3]].filter((axis) => axis !== undefined),
    [shared[4]],
  );
  return creationListingLengthTokens(text)
    .slice(0, 3)
    .map((token) => creationListingDimensionAxisMillimeters(token.number, token.unit))
    .filter((axis) => axis !== null);
}

function roundCreationListingDimensionMillimeters(value) {
  if (!Number.isFinite(value) || value <= 0) return 1;
  const increment = value >= 100 ? 5 : value >= 25 ? 2 : 1;
  return Math.max(increment, Math.round(value / increment) * increment);
}

function completeCreationListingDimensionAxes(axes = [], source = {}, kind = "product") {
  const profile = creationListingDimensionProfile(source, kind);
  const completed = axes.slice(0, 3);
  if (completed.length === 0 && kind === "package") {
    const productAxes = firstCreationListingDimensionAxes(
      getCreationListingDimensionEvidence(source, "product"),
    );
    if (productAxes.length > 0) {
      const length = Math.max(
        productAxes[0] * profile.packageScale,
        productAxes[0] + profile.packageClearanceMm,
      );
      const width = productAxes[1]
        ? Math.max(
          productAxes[1] * profile.packageScale,
          productAxes[1] + profile.packageClearanceMm * 0.65,
          profile.minimumPackageWidth,
        )
        : Math.max(length * profile.widthRatio, profile.minimumPackageWidth);
      const height = productAxes[2]
        ? Math.max(
          productAxes[2] * profile.packageScale,
          productAxes[2] + profile.packageClearanceMm * 0.5,
          profile.minimumPackageDepth,
        )
        : Math.max(Math.min(length, width) * profile.depthRatio, profile.minimumPackageDepth);
      return [length, width, height].map(roundCreationListingDimensionMillimeters);
    }
  }
  if (completed.length === 0) return profile.base.map(roundCreationListingDimensionMillimeters);
  if (completed.length === 1) {
    completed.push(roundCreationListingDimensionMillimeters(Math.max(
      completed[0] * profile.widthRatio,
      kind === "package" ? profile.minimumPackageWidth : 0,
    )));
  }
  if (completed.length === 2) {
    completed.push(roundCreationListingDimensionMillimeters(Math.max(
      Math.min(...completed) * profile.depthRatio,
      kind === "package" ? profile.minimumPackageDepth : 0,
    )));
  }
  return completed;
}

function formatCreationListingDimensionNumber(value, divisor) {
  const number = Math.round((value / divisor) * 100) / 100;
  return String(number).replace(/\.0$/u, "");
}

function formatCompletedCreationListingDimensions(axes = [], source = {}, language = "english") {
  const mode = cleanString(source.dimensionUnitMode).toLowerCase() || "both";
  const chinese = language === "chinese";
  const metric = `${axes.map((axis) => formatCreationListingDimensionNumber(axis, 10)).join(chinese ? " × " : " x ")} ${chinese ? "厘米" : "cm"}`;
  const imperial = `${axes.map((axis) => formatCreationListingDimensionNumber(axis, 25.4)).join(chinese ? " × " : " x ")} ${chinese ? "英寸" : "in"}`;
  if (mode === "metric") return metric;
  if (mode === "imperial") return imperial;
  return chinese ? `${metric}（${imperial}）` : `${metric} (${imperial})`;
}

function completedCreationListingPackageDimensions(value, source = {}, language = "english") {
  const evidence = getCreationListingDimensionEvidence(source, "package");
  const rawValue = cleanString(value);
  const valueIsEstimate = /^(?:Estimated\s*:|预估\s*[:：])/iu.test(rawValue);
  const candidate = evidence || rawValue;
  const axes = firstCreationListingDimensionAxes(candidate);
  if (!evidence && axes.length >= 3) {
    if (valueIsEstimate) return rawValue;
    const dimensions = Array.from(rawValue).length <= CREATION_LISTING_FIELD_MAX_CHARS
      ? formatCompletedCreationListingDimensions(axes, source, language)
      : rawValue;
    return language === "chinese" ? `预估：${dimensions}` : `Estimated: ${dimensions}`;
  }
  const dimensions = formatCompletedCreationListingDimensions(
    completeCreationListingDimensionAxes(axes, source, "package"),
    source,
    language,
  );
  if (evidence) return dimensions;
  return language === "chinese" ? `预估：${dimensions}` : `Estimated: ${dimensions}`;
}

function estimatedCreationListingDimensionValues(source = {}, kind = "product") {
  const axes = completeCreationListingDimensionAxes([], source, kind);
  return {
    english: formatCompletedCreationListingDimensions(axes, source, "english"),
    chinese: formatCompletedCreationListingDimensions(axes, source, "chinese"),
  };
}
...
function buildFallbackCreationListingDimensionFields(source = {}) {
  const build = (kind) => {
    ...
    if (evidence) {
      if (kind === "package") {
        const english = completedCreationListingPackageDimensions(evidence, source, "english");
        const chinese = completedCreationListingPackageDimensions(evidence, source, "chinese");
    ...
    if (kind === "package") {
      return {
        english: `Estimated: ${estimatedCreationListingDimensionValues(source, kind).english}`,
        chinese: `预估：${estimatedCreationListingDimensionValues(source, kind).chinese}`,
      };
    }
...
function applyCreationListingPackageDimensions(draft = {}, source = {}) {
  const zhDisplay = draft.zhDisplay && typeof draft.zhDisplay === "object" && !Array.isArray(draft.zhDisplay)
    ? draft.zhDisplay
    : {};
  return {
    ...draft,
    packageDimensions: completedCreationListingPackageDimensions(draft.packageDimensions, source, "english"),
    zhDisplay: {
      ...zhDisplay,
      packageDimensions: completedCreationListingPackageDimensions(zhDisplay.packageDimensions, source, "chinese"),
    },
  };
}
...
function creationListingWeightProfileText(source = {}) {
  return [
    source.parentProductName,
    source.setProductName,
    source.productName,
    ...
    source.dimensionSpecs,
    source.dimensions,
    source.industryTemplatePath,
    source.skuNote,
    source.packageContents,
    source.packageList,
    ...(Array.isArray(source.sellingPoints) ? source.sellingPoints : []),
    ...(Array.isArray(source.referenceImageRoles)
      ? source.referenceImageRoles.flatMap((item) => [item?.role, item?.title, item?.note])
      : []),
    ...(Array.isArray(source.skuSubjects)
      ? source.skuSubjects.flatMap((item) => [item?.title, item?.description, item?.note])
      : []),
  ].map(cleanString).filter(Boolean).join(" " ).toLowerCase();
}

function creationListingEstimatedUnitCount(source = {}) {
  const directCount = positiveIntegerCount(source.skuBundleCount);
  const quantityOptions = uniquePositiveIntegerCounts(
    source.skuQuantityOptions ?? source.sku_quantity_options ?? [],
  );
  const subjectCounts = Array.isArray(source.skuSubjects)
    ? source.skuSubjects.map(skuSubjectListingQuantity).filter((count) => count > 0)
    : [];
  return Math.max(1, directCount, ...quantityOptions, ...subjectCounts);
}

function roundCreationListingEstimatedWeightGrams(value) {
  const grams = Math.max(10, Math.min(25000, Number(value) || 0));
  const increment = grams >= 1000 ? 50 : grams >= 300 ? 10 : 5;
  return Math.max(increment, Math.round(grams / increment) * increment);
}

function estimatedCreationListingProductWeightGrams(source = {}) {
  const text = creationListingWeightProfileText(source);
  const profiles = [
    { pattern: /(?:qipao|cheongsam|旗袍)/u, grams: 320 },
    { pattern: /(?:dress|gown|连衣裙|礼服裙|裙装)/u, grams: 360 },
    { pattern: /(?:hoodie|sweatshirt|卫衣|连帽衫)/u, grams: 600 },
    { pattern: /(?:shirt|blouse|t-shirt|tee|衬衫|上衣|短袖)/u, grams: 230 },
    { pattern: /(?:jacket|coat|blazer|夹克|外套|大衣|西装)/u, grams: 750 },
    { pattern: /(?:skirt|短裙|半身裙)/u, grams: 280 },
    { pattern: /(?:pants|trousers|jeans|裤|牛仔裤)/u, grams: 520 },
    { pattern: /(?:bottle|tumbler|cup|flask|水瓶|水杯|保温杯)/u, grams: 300 },
    { pattern: /(?:goggle|glasses|eyewear|护目镜|眼镜|风镜)/u, grams: 120 },
    { pattern: /(?:lure|bait|minnow|fishing|路亚|鱼饵|拟饵)/u, grams: 45 },
    { pattern: /(?:charger|camera|screen|remote|battery|electronic|充电器|相机|显示器|遥控|电池|电子)/u, grams: 280 },
    { pattern: /(?:tool|latch|lock|hardware|hinge|工具|插销|锁|五金|铰链)/u, grams: 420 },
    { pattern: /(?:bag|handbag|backpack|pouch|包|背包|手提包|收纳袋)/u, grams: 380 },
    { pattern: /(?:paper|card|sticker|film|sheet|纸|卡片|贴纸|薄膜)/u, grams: 80 },
  ];
  const profile = profiles.find((candidate) => candidate.pattern.test(text));
  let grams = profile?.grams ?? 250;

  if (/(?:silk|satin|chiffon|lace|薄款|轻薄|真丝|仿真丝|缎|雪纺|蕾丝)/u.test(text)) {
    grams *= /(?:qipao|cheongsam|旗袍)/u.test(text) ? 1 : 0.9;
  } else if (/(?:denim|wool|leather|velvet|quilted|厚款|加绒|牛仔|羊毛|皮革|丝绒|夹棉)/u.test(text)) {
    grams *= 1.3;
  }
  if (/(?:bead|sequin|embroidery|珠绣|钉珠|亮片|重工刺绣)/u.test(text)
    && !/(?:no|without|无|没有|不含)[^。.;；]{0,8}(?:bead|sequin|embroidery|珠绣|钉珠|亮片|重工刺绣)/u.test(text)) grams *= 1.15;
  if (/(?:lined|lining|里衬|内衬)/u.test(text)
    && !/(?:unlined|no\s+lining|without\s+lining|无里衬|无内衬|没有里衬|不含里衬)/u.test(text)) grams *= 1.12;

  const axes = firstCreationListingDimensionAxes(getCreationListingDimensionEvidence(source, "product"));
  const longestAxis = axes.length > 0 ? Math.max(...axes) : 0;
  if (longestAxis >= 1000 && /(?:qipao|cheongsam|dress|gown|旗袍|连衣裙|礼服裙)/u.test(text)) {
    grams *= Math.min(1.18, 1 + ((longestAxis - 1000) / 2500));
  } else if (longestAxis > 0 && longestAxis <= 100 && !/(?:paper|card|sticker|film|纸|卡片|贴纸|薄膜)/u.test(text)) {
    grams *= 0.8;
  }

  const unitCount = creationListingEstimatedUnitCount(source);
  grams *= unitCount;
  return roundCreationListingEstimatedWeightGrams(grams);
}

function estimatedCreationListingWeightGrams(source = {}, kind = "product") {
  const productGrams = estimatedCreationListingProductWeightGrams(source);
  if (kind !== "package") return productGrams;
  const text = creationListingWeightProfileText(source);
  const softGoods = /(?:qipao|cheongsam|dress|shirt|blouse|hoodie|skirt|pants|clothing|apparel|fabric|textile|bag|旗袍|连衣裙|衬衫|卫衣|裙|裤|服装|衣|布|纺织|包)/u.test(text);
  const paperGoods = /(?:paper|card|sticker|film|sheet|纸|卡片|贴纸|薄膜)/u.test(text);
  const fragileGoods = /(?:glass|ceramic|bottle|goggle|glasses|camera|screen|玻璃|陶瓷|水瓶|眼镜|相机|显示器)/u.test(text);
  const overhead = paperGoods
    ? Math.max(25, productGrams * 0.18)
    : softGoods
      ? Math.max(100, productGrams * 0.22)
      : fragileGoods
        ? Math.max(140, productGrams * 0.35)
        : Math.max(80, productGrams * 0.28);
  return roundCreationListingEstimatedWeightGrams(productGrams + overhead);
}

function formatCreationListingEstimatedWeight(grams, mode) {
  const metric = grams >= 1000
    ? `${Number((grams / 1000).toFixed(2))} kg`
    : `${grams} g`;
  const imperial = grams >= 453.59237
    ? `${Number((grams / 453.59237).toFixed(2))} lb`
    : `${Number((grams / 28.349523125).toFixed(2))} oz`;
  if (mode === "metric") return { english: metric, chinese: metric.replace(/kg\b/gu, "千克").replace(/g\b/gu, "克") };
  if (mode === "imperial") return { english: imperial, chinese: imperial.replace(/lb\b/gu, "磅").replace(/oz\b/gu, "盎司") };
  return {
    english: `${metric} (${imperial})`,
    chinese: `${metric.replace(/kg\b/gu, "千克").replace(/g\b/gu, "克")}（${imperial.replace(/lb\b/gu, "磅").replace(/oz\b/gu, "盎司")}）`,
  };
}

function estimatedCreationListingWeightValues(source = {}, kind = "product") {
  const mode = cleanString(source.dimensionUnitMode).toLowerCase() || "both";
  return formatCreationListingEstimatedWeight(
    estimatedCreationListingWeightGrams(source, kind),
    mode,
  );
}
...
function applyCreationListingWeightFields(draft = {}, source = {}) {
  ...
  const zhDisplay = draft.zhDisplay && typeof draft.zhDisplay === "object" && !Array.isArray(draft.zhDisplay)
    ? draft.zhDisplay
    : {};
  const resolve = (kind, englishValue, chineseValue) => {
    const evidence = getCreationListingWeightEvidence(source, kind);
    if (evidence) {
      return kind === "package"
        ? { english: fallback.packageWeight, chinese: fallback.zhPackageWeight }
        : { english: fallback.productWeight, chinese: fallback.zhProductWeight };
    }
    const english = cleanString(englishValue);
    const chinese = cleanString(chineseValue);
    const englishEstimate = /^(?:Estimated\s*:)/iu.test(english) && /\d\s*(?:kg|g|lb|lbs|oz)\b/iu.test(english);
    const chineseEstimate = /^预估\s*[:：]/u.test(chinese) && /\d\s*(?:千克|公斤|克|磅|盎司)/u.test(chinese);
    if (englishEstimate && chineseEstimate) return { english, chinese };
    return kind === "package"
      ? { english: fallback.packageWeight, chinese: fallback.zhPackageWeight }
      : { english: fallback.productWeight, chinese: fallback.zhProductWeight };
  };
  const packageValue = resolve("package", draft.packageWeight, zhDisplay.packageWeight);
  const productValue = resolve("product", draft.productWeight, zhDisplay.productWeight);
  return {
    ...draft,
    packageWeight: packageValue.english,
    productWeight: productValue.english,
    zhDisplay: {
      ...zhDisplay,
      packageWeight: packageValue.chinese,
      productWeight: productValue.chinese,
    },
  };
}
...
export function hydrateCreationListingDimensionsForRead(set = {}) {
  ...
  const listingDrafts = set.listingDrafts.map((draft) => {
    ...
    const packageValue = cleanString(draft.packageDimensions ?? draft.package_dimensions);
    const packageDimensions = packageValue
      ? completedCreationListingPackageDimensions(packageValue, set, "english")
    ...
    const zhPackageValue = cleanString(zhDisplay.packageDimensions ?? zhDisplay.package_dimensions);
    const zhPackageDimensions = zhPackageValue
      ? completedCreationListingPackageDimensions(zhPackageValue, set, "chinese")
...
function makeDeterministicCompletedDraft(source = {}, policy = {}, validationErrors = []) {
  ...
  return finalizeListingDraftTitlePartNumbers(validation.draft, source);
}
...
export function makeMockCreationListingDraft(source = {}) {
  ...
  return finalizeListingDraftTitlePartNumbers(normalizeCompletedLegacyDraft({
...
export async function requestCreationListingDraft({
  ...
  if (requestContext.legacyContract && requestSource.forceV1 === true) {
    const parsed = await requestUsableListingResponse({
      baseUrl,
      endpointPath,
      apiKey,
      responsesModel,
    ...
    let draft = sanitizeCompletedListingDraftClaims(normalizedDraft, requestSource, normalizedDraft);
    draft = finalizeListingDraftTitlePartNumbers(draft, requestSource);
    draft = completeListingDraftTitles(draft, requestSource);
    return finalizeListingDraftTitlePartNumbers(draft, requestSource);
  }
  ...
  return normalizeParsedListingDraft(
    parsed,
    requestSource,
