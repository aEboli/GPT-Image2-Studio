...
function isExpectedV2ContentType(value, field) {
  if (["title", "description"].includes(field)) return typeof value === "string";
...
function bilingualFactAnchors(value) {
  ...
  for (const match of text.matchAll(/\b(?:mm|cm|kg|ml|l|in|ft|oz|lb)\b|毫米|厘米|千克|公斤|毫升|升|英寸|英尺|盎司|磅/giu)) {
    const unit = match[0].toLocaleLowerCase("und");
    const normalizedUnit = new Map([
      ["毫米", "mm"], ["厘米", "cm"], ["千克", "kg"], ["公斤", "kg"], ["毫升", "ml"],
