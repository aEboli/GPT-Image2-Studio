...
- **AND** existing close, download, delete, copy path, and copy prompt behavior remains stable
...
The system SHALL preserve existing lightbox detail actions while adding viewer interactions.

#### Scenario: User uses existing lightbox actions after zooming
- **WHEN** the user has zoomed or panned a lightbox image
- **THEN** download, delete, copy prompt, copy relative path, copy full path, backdrop close, close button, and Esc close continue to perform their existing actions
