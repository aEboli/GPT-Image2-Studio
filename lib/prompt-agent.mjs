...
export function extractPromptAgentJson(payload, rawText = "") {
  ...
  for (const candidate of candidates) {
    try {
      return normalizePromptAgentJson(parseJsonObject(candidate));
    } catch (error) {
      lastError = error;
    }
  }
...
export async function consumePromptAgentSse(stream) {
  ...
  const finish = () => {
    ...
    for (const candidate of pickStreamText({ completedText, chatMessageText, collectedTexts, deltaText })) {
      try {
        return extractPromptAgentJson(candidate);
      } catch (error) {
        lastError = error;
      }
    }
  ...
  const processChunk = (chunk) => {
    ...
    if (trimmedChunk && !/^event\s*:|^data\s*:/m.test(trimmedChunk) && /^[\[{]/.test(trimmedChunk)) {
      try {
        return extractPromptAgentJson(JSON.parse(trimmedChunk));
      } catch (error) {
...
function extractPromptAgentResponseText(responseText) {
  let payload;
  try {
  ...
  return extractPromptAgentJson(payload, responseText);
}
...
async function parsePromptAgentResponse(response) {
  ...
  if (looksLikePromptAgentSse(responseText)) {
    return consumePromptAgentSse(createPromptAgentTextStream(responseText));
  }

  return extractPromptAgentResponseText(responseText);
}
