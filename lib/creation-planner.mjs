...
export const MAX_CREATION_EFFECTIVE_PLAN_BYTES = 750_000;
...
export const CREATION_IMAGE_COUNT_OPTIONS = [0, 4, 6, 7, 8, 9, 10, 12, 14, 16, 18];
...
function getCreationPlatformSlotTitle(slot = {}, role = {}) {
...
export function buildCreationPlan(input = {}) {
  ...
  const carouselItems = plannedRoles.map((role, index) => {
    ...
    const generatedPrompt = [
      platformPolicyDominant
        ? `Create ${platformSlot.imageTypeLabel || platformSlot.imageType} as a policy-led marketplace asset.`
        : `Create ${role.brief}.`,
      `Product: ${productLine}.`,
    ...
    return {
      ...
      itemKind: "carousel",
      role: role.role,
      title: getCreationPlatformSlotTitle(platformSlot, role),
  ...
  const editableSlots = legacyPlanningMode
    ? carouselItems
    : platformPlan.slots.map((slot) => carouselItemsBySlotKey.get(slot.slotKey) || {
      ...slot,
      itemId: slot.itemId || slot.slotKey,
      itemKind: "carousel",
      title: getCreationPlatformSlotTitle(slot, getCreationPlatformRoleDefinition(slot.role)),
      prompt: slot.prompt || "",
    });
