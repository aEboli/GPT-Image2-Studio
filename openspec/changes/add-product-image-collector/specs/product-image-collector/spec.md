...
The collector, launcher, and collector panel SHALL load into the page's extension isolated world as declared content scripts. On load the collector and panel SHALL only register their in-page controllers and MUST NOT create collector DOM, inspect product media, fetch product data, or contact the background. Clicking the launcher SHALL open the already-loaded panel through its controller within the page, without sending an open message to the background and without any open timeout. Opening an unmounted panel SHALL reset the previous collection, selection, and busy state, then invoke the already-loaded collector controller in the same isolated world; the normal product-page read MUST NOT depend on a background message or Service Worker response. Opening an already-mounted panel SHALL only reveal it without recollecting. Closing SHALL detach the panel and restore the launcher while retaining both controllers so a later click still opens and collects in-page; only a version mismatch SHALL replace the affected controller. Because one panel controller serves repeated mounts, a result that arrives after its own mount closed MUST NOT write collection, status, or busy state into a later mount. The background open and collect messages SHALL remain only as fallbacks for pages whose isolated world lacks a current controller. The open fallback SHALL retry a small fixed number of bounded attempts, re-attempt the in-page controller between attempts, and report a refresh instruction instead of a bare timeout once every attempt fails. Background injection SHALL explicitly invoke the current panel or collector controller after injecting its files so browser-action and legacy-page paths remain deterministic.
...
The right title block SHALL be a keyboard-operable theme button that cycles deterministically through Day, Blue, and Night themes. A compact Sun, Droplet, or Moon icon SHALL appear after the current platform label to identify the active theme, while an accessible name and tooltip SHALL name both the active theme and the next theme. Every theme SHALL adapt the panel header, selection controls, status, group hierarchy, cards, named SKU row, footer, notices, scrollbars, and viewer controls with readable contrast. The Blue theme SHALL use a visibly mid-dark navy blue-hour palette with cool-white text, ordered blue group surfaces, a bright cyan selection accent, and a contrasting orange primary action; it MUST NOT appear as a lightly blue-tinted copy of the Day theme or as the near-black neutral Night theme. Theme changes SHALL update the existing panel in place and MUST NOT rebuild image cards or change collection, selection, scroll, folding, docking, or viewer state. The thumbnail media background and black translucent metadata strip SHALL remain stable across themes so source-image review does not change.
...
- **AND** the collector and collector panel register their in-page controllers without creating collector DOM, reading product images, fetching product data, or contacting the background
- **AND** its first click opens the current collector panel in-page without any prior extension-action click and without sending an open message to the background
- **AND** the panel reads the product page through the in-page collector controller without waiting for a Service Worker response
...
- **AND** selected cards have a visible teal border, card and group-selection checkboxes use the same teal accent, unselected cards retain a neutral border, and complete compact names plus resolutions share one black translucent metadata strip with white text
...
#### Scenario: Header separates product title and source platform
- **WHEN** the collected manifest title ends in the normalized suffix `——1688`
- **THEN** the left header shows the complete product-title portion without that suffix
- **AND** the centered right title block shows `GPT-Image2-Studio` and `1688`
- **AND** the manifest title, copied Studio payload, product folder, and downloaded filenames retain their existing normalized source data

#### Scenario: User cycles all collector themes
- **WHEN** the user repeatedly activates the brand and platform theme button
- **THEN** the panel cycles in order from Day to Blue to Night and back to Day
- **AND** the platform-line icon, tooltip, accessible name, and all themed panel surfaces reflect the current theme
- **AND** Blue presents a mid-dark navy blue-hour hierarchy with cool-white text, bright cyan selection, and an orange primary action that is visibly distinct from both Day and Night
- **AND** existing image-card nodes, selection, list scroll position, fold state, dock edge, and an open viewer remain unchanged
...
- **THEN** the black translucent lower metadata strip shows a complete compact category-order label on the left and `<width>×<height>` on the right in white text
- **AND** GigaB2B uses bounded `image/info` metadata instead of the 300px card preview dimensions
- **AND** the metadata background remains translucent enough for the covered source image to stay recognizable without losing white-text readability
...
#### Scenario: Square thumbnail keeps a white inset in the collector card
- **WHEN** a collected image has a 1:1 aspect ratio
- **THEN** its card image element is centered at 98 percent of the square media area's width and height
- **AND** approximately 1 percent of the pure white media background remains visible on every edge without changing the card, metadata strip, checkbox, or action dimensions
...
- **AND** the variant row uses fixed 12 CSS pixel bold type, normal wrapping, and anywhere overflow wrapping instead of runtime font shrinking
- **AND** it uses the natural wrapped-content height without reserving empty lines for short names and grows when needed, while every card action strip in the same grid row remains bottom-aligned
- **AND** Day, Blue, and Night use visibly distinct high-contrast yellow-orange, bright-cyan, and magenta variant palettes respectively
...
- **AND** no SKU label receives an inline font size or becomes smaller than the fixed readable size
...
#### Scenario: User navigates to a product through a storefront SPA
- **WHEN** the content script starts on an authorized non-product storefront route and the page changes to a supported consumer product-detail URL without a document reload
- **THEN** a working floating launcher appears without requiring the browser-toolbar action
- **AND** the script does not inspect product images, create the collector panel, or send a collection request before the user clicks that launcher
- **AND** leaving the supported product route removes the launcher and closes any collector that belongs to the previous product

#### Scenario: The floating launcher cannot reach the background
- **WHEN** the page's isolated world has no current panel controller and the user clicks the launcher while the runtime throws, reports an error, or does not answer within the bounded open timeout
- **THEN** the launcher retries the bounded open attempt a small fixed number of times and re-attempts the in-page controller between attempts
- **AND** the launcher becomes enabled again and instructs the user to refresh the page instead of remaining permanently disabled or reporting a bare timeout
- **AND** the user can retry the same launcher without first clicking the browser-toolbar action

#### Scenario: A collection reply arrives after its own collector closed
- **WHEN** the user closes the collector while a collection request is still pending and reopens it on the same page
- **THEN** the reopened collector starts from a cleared collection and selection and issues its own collection request
- **AND** the late reply from the closed collector does not change the reopened collector's images, selection, status, or busy state
...
#### Scenario: A long transfer is pending
- **WHEN** direct image copy or download is still awaiting its bounded background response
- **THEN** commands that could start another transfer or recollection remain disabled
- **AND** selection, image viewing, folding, and closing remain responsive instead of making the whole collector appear frozen
- **AND** a timeout restores the transfer commands and reports an explicit retryable error while any late callback is ignored by the settled UI request
...
### Requirement: Extension copies selected images as one Windows file list
The extension SHALL expose a distinct `复制图片` command between `复制到 Studio` and `下载所选`. The collector manifest and direct-copy selection SHALL support up to 1000 normalized image items in one bounded Native Messaging request of at most 4 MiB. The command SHALL send only the current normalized selection to the dedicated Windows host `com.aeboli.gpt_image2_studio.product_image_clipboard` and SHALL NOT require Studio to be running or open. The host SHALL be registered only for the stable extension origin, accept no arbitrary command or filesystem path, and reapply the trusted source, image host, redirect, type, per-image size, 512 MiB aggregate size, and timeout boundaries before saving successful images to one temporary batch. On Windows it SHALL dispatch the final clipboard operation to a dedicated STA thread after asynchronous image preparation, retry transient clipboard contention within a fixed bound, and atomically write all successful absolute image paths as one FileDrop clipboard list. It MUST NOT emulate the operation with multiple browser `ClipboardItem` values, Base64 HTML, a composite image, arbitrary loopback-port scanning, general-purpose IPC, or a silent first-image-only fallback. This collector batch limit MUST NOT increase the Creation generation limit of 15 reference images.

#### Scenario: User copies multiple selected images for chat
- **WHEN** the user selects multiple collected images and invokes `复制图片` after installing the dedicated clipboard host
- **THEN** the dedicated host prepares every successfully validated image as a separate temporary file and writes one FileDrop list containing those files
- **AND** the extension reports the successful count so the user can paste the images into a chat application in one action
- **AND** the collector briefly shows a semi-transparent in-panel success notice that fades in and out without moving the image list or footer controls

#### Scenario: User copies another image batch before the success notice expires
- **WHEN** a successful `复制图片` response arrives while the previous success notice is still visible
- **THEN** the notice updates to the latest successful count and restarts its fade timing
- **AND** detailed partial-failure counts remain available in the persistent status row

#### Scenario: Studio is closed during direct image copy
- **WHEN** the dedicated clipboard host is installed and no Studio browser tab, server, or desktop window is running
- **THEN** the extension still prepares the selected images and writes the FileDrop clipboard list
- **AND** it does not query loopback tabs or call a Studio HTTP endpoint

#### Scenario: User copies a batch containing hundreds of images
- **WHEN** the normalized manifest and selected IDs contain up to 1000 valid images and remain within the 4 MiB request and 512 MiB image-total boundaries
- **THEN** every successfully validated image is represented as an independent file in the one FileDrop clipboard list
- **AND** no two-image or 120-image implementation cap truncates the selection

#### Scenario: Some selected images cannot be prepared
- **WHEN** one or more selected image fetches fail but at least one validated image succeeds
- **THEN** the clipboard contains only the successful image files
- **AND** the extension separately reports successful and failed counts

#### Scenario: Asynchronous preparation resumes outside STA
- **WHEN** image downloads complete on a worker-thread continuation before the FileDrop write
- **THEN** the host dispatches the complete clipboard operation to a dedicated STA thread
- **AND** retries only transient clipboard contention within a fixed bound before reporting a busy error

#### Scenario: Direct image copy is unavailable
- **WHEN** the runtime is not Windows, the dedicated host is not installed, no item is selected, all selected images fail, or the native clipboard write fails
- **THEN** the extension displays a specific error and does not claim that images were copied
- **AND** it does not fall back to copying only the first image or replacing the selection with a composite
...
The extension SHALL use Manifest V3 and SHALL declare HTTPS host permissions only for the supported 1688, Amazon, Temu, TikTok Shop, SHEIN, and GigaB2B storefront domains. It MAY declare `nativeMessaging` only for the explicit `复制图片` action and MUST NOT declare loopback HTTP host permissions. Automatic launcher and collector-panel content scripts MAY cover the already-authorized storefront host scope when required to observe in-site SPA navigation and to open the panel without a background round trip, but SHALL perform complete runtime URL validation before creating launcher DOM and SHALL NOT create collector DOM, inspect product media, or contact the background on a non-product route or before the user opens the collector. Runtime validation SHALL reject category, search, homepage, seller-center, account, and unsupported regional pages. The action handler SHALL use the clicked tab directly, and launcher/panel requests SHALL use `sender.tab.id` plus the current `location.href` instead of querying for an active tab or depending on `Tab.url` visibility. It MUST NOT request `<all_urls>`, `tabs`, or `sidePanel`, query loopback tabs, scan arbitrary local ports, read browsing history, capture authentication data, bypass login, CAPTCHA, risk checks or platform protections, or collect in the background without a user action.
