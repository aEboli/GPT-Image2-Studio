...
export function buildCreationQueuedSet({
  ...
  if (frozenEffectivePlan && Array.isArray(frozenEffectivePlan.items) && frozenEffectivePlan.items.length > 0) {
    ...
    return normalizeSet({
      ...
      dimensionSpecs: snapshot.dimensionSpecs || "",
      ...
      skuBundleCount: normalizeCreationSkuBundleCountForPayload(snapshot.skuBundleCount || "1"),
  ...
  return normalizeSet({
    ...
    infographicRebuildCount: infographicRebuildSources.length,
    skuSubjects,
    skuBundleCount: normalizeCreationSkuBundleCountForPayload(draftSet?.skuBundleCount || "1"),
