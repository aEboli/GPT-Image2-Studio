...
- **Gemini 生图路径只支持 15 个比例中的 10 个。** 它接受 `1:1`、`2:3`、`3:2`、`3:4`、`4:3`、`4:5`、`5:4`、`9:16`、`16:9` 和 `21:9`。选择 `9:21`、`2:1`、`1:2`、`3:1` 或 `1:3` 时，实际发给上游的比例是 `1:1`，因为档位字符串不带宽高，就近匹配比例的兜底逻辑没有可用的尺寸依据。提示词里仍然带着原始比例描述，所以明明请求横幅，结果却可能是方图。
...
每个版本的完整说明、产物校验值和验证记录同时发布在 [GitHub Releases](https://github.com/aEboli/GPT-Image2-Studio/releases)。
