...
const CREATION_ROLE_INTENT_INSTRUCTIONS = {
  ...
  "effect-comparison":
    "Role intent: create one product-led functional effect rendering in a single unified composition. Keep one dominant, fully visible product as the visual anchor. Show every supported product function, mechanism, effect path, or outcome with dimensional arrows, cutaway-style overlays, motion trails, energy or material-flow cues, and premium 3D/CGI rendering when useful. Do not use before-and-after, comparison, side-by-side, split-screen, paired-state, duplicate-product, or multi-panel layouts. Do not invent unsupported technical structures, performance numbers, certifications, or effects.",
  "spec-table":
...
const CREATION_ROLE_RENDERING_CONSTRAINTS = {
  ...
  "effect-comparison":
    "For this functional effect rendering, keep one dominant, fully visible product in one unified composition and show effect cues only around that same product. Do not use before-and-after, comparison, side-by-side, split-screen, paired-state, duplicate-product, or multi-panel layouts.",
  "spec-table":
...
function buildCreationSkuItemDisplayName(skuSubject = {}, index = 0, targetLanguage = {}) {
  ...
  const fallbackName = cleanString(skuSubject.title || skuSubject.id || skuSubject.filenames?.[0]);
  const name = colorName || fallbackName;
  return name ? `SKU image ${skuIndex} - ${name}` : `SKU image ${skuIndex}`;
}
...
function buildCreationSuiteSplitGuidance(role, slotIndex, totalCount) {
  ...
  const functionalEffectAntiRedundancy =
    "For this functional effect rendering, use one unified product-led composition with one dominant, fully visible product and show all supported functional cues around it. Do not use before-and-after, comparison, side-by-side, split-screen, paired-state, duplicate-product, or multi-panel layouts.";
  ...
  const carouselItems = plannedRoles.map((role, index) => {
    ...
    const roleGuidance = platformPolicyDominant
      ...
      : [
          ...
          platformNativeSlot && role.role !== "effect-comparison" ? "" : getCreationRoleIntentInstruction(role.role),
          ...
          buildCreationVisualLanguageGuidance(visualLanguage),
          platformNativeSlot && !["spec-table", "accessory-gift", "effect-comparison"].includes(role.role)
            ? ""
