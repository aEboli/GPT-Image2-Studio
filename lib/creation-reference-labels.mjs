...
function getReferenceRoleIdentityInstruction(role = null, { primaryAnchor = false } = {}) {
  ...
  if (isCreationSubjectReferenceRole(roleValue) || roleLabel.includes("product subject") || roleLabel.includes("reference subject")) {
    return primaryAnchor
      ...
      : "Supporting product evidence only; keep the primary identity fixed.";
  }
  return "Assigned reference evidence only.";
}
...
export function buildCreationReferenceImageLabels(referenceImages = [], referenceImageRoles = []) {
  ...
  return names.map((name, index) => {
    ...
    const roleValue = cleanString(role?.role).toLowerCase();
    const roleLabel = roleValue === "reference-product"
      ? "reference subject"
      : roleValue || cleanString(role?.rolePromptLabel || role?.promptLabel || role?.roleLabel || "supporting reference");
    const note = cleanString(role?.note || role?.analysisNote || role?.description)
      .replace(/[.!?。！？]+$/u, "");
    const isPrimary = index === primaryIndex;
    const customInstruction = !cleanString(role?.role)
      ? cleanString(role?.promptInstruction || role?.instruction).replace(/[.!?。！？]+$/u, "")
      : "";
    const identityInstruction = getReferenceRoleIdentityInstruction(role, { primaryAnchor: isPrimary });
    const roleSummary = role
      ? `Role: ${roleLabel}.${customInstruction ? ` ${customInstruction}.` : ""}${note ? ` Note: ${note}.` : ""}`
      : "";
    return [
