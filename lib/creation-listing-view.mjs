...
function createCreationListingValueCopyTarget(displayText, copyText, label, { localized = false } = {}) {
  const target = document.createElement("button");
  target.className = localized ? "creation-listing-localized" : "creation-listing-value-copy";
  target.type = "button";
...
function appendLocalizedText(parent, localizedText, { copyText = localizedText, label = "中文参考" } = {}) {
  if (!localizedText) {
    return;
  }
  parent.appendChild(createCreationListingValueCopyTarget(localizedText, copyText, label, { localized: true }));
}
...
function createCreationListingField(label, value, {
  ...
  if (copyable) {
    ...
    labelNode.title = `点击复制${label}英文`;
    labelNode.setAttribute("aria-label", `复制${label}英文`);
    applyCreationListingCopyData(labelNode, `${label}英文`, copySource, { list: copyList });
  }
  ...
  const rows = buildCreationListingFieldRows(value, localizedValue, { list });
  ...
  const appendRowCopyTargets = (parent, row, index) => {
    const itemLabel = list ? `${label}第 ${index + 1} 条` : label;
    const rowCopyText = copyRows.length === rows.length ? copyRows[index] : row.text;
    parent.appendChild(createCreationListingValueCopyTarget(
      row.text,
      rowCopyText || row.text,
      `${itemLabel}英文`,
    ));
    appendLocalizedText(parent, row.localizedText, {
      copyText: localizedCopyRows[index] || row.localizedText,
      label: `${itemLabel}中文`,
    });
  };
  ...
  if (list) {
    ...
    rows.forEach((item, index) => {
      ...
      appendRowCopyTargets(row, item, index);
  ...
  appendRowCopyTargets(copy, row, 0);
...
export function renderCreationListingDrafts({ refs, state, set } = {}) {
  ...
  drafts.forEach((draft, index) => {
    ...
    titleCopy.title = "点击复制标题英文";
    titleCopy.setAttribute("aria-label", "复制标题英文");
    applyCreationListingCopyData(titleCopy, "标题英文", headerContent.title);
...
export function createCreationListingController(context = {}) {
  ...
  function syncRecordControls(selectedSet) {
    ...
    if (context.refs.creationRecordGenerateListingsButton) {
      ...
      context.refs.creationRecordGenerateListingsButton.title = "";
    }
