...
const PACKAGE_DIMENSION_LABEL_PATTERN = /(?:package|packaging|packed|shipping|carton|color[-\s]?box|outer[-\s]?box)\s*(?:size|dimensions?|measurements?|[:：])|(?:包装|装箱|彩盒|外箱|运输箱)(?:尺寸|大小|规格|[:：])/iu;
...
export function getCreationListingDimensionEvidence(source = {}, kind = "product") {
  ...
  const formatEvidence = (value) => compactDimensionEvidence(
    formatListingDimensionTextForMode(value, dimensionUnitMode),
  );
...
export function getCreationListingWeightFieldErrors(draft = {}, source = {}) {
  ...
  for (const [field, kind] of [["packageWeight", "package"], ["productWeight", "product"]]) {
    ...
    if (evidence) {
      ...
      if (english && !sharesListingDimensionNumber(english, evidence)) {
        errors.push(`${field} must retain a numeric anchor from its source evidence`);
      }
      if (chinese && !sharesListingDimensionNumber(chinese, evidence)) {
...
function matchesCompleteListingDimensionEvidence(value, evidence) {
  const allEvidenceTuples = uniqueListingDimensionTuples(listingDimensionTuples(evidence));
  const evidenceTuples = allEvidenceTuples.filter((tuple) => !(
    tuple.values.length === 1
    && allEvidenceTuples.some((candidate) => (
  ...
  const valueTuples = uniqueListingDimensionTuples(listingDimensionTuples(value));
...
export function getCreationListingDimensionFieldErrors(draft = {}, source = {}) {
  ...
  for (const [field, kind] of [["packageDimensions", "package"], ["productDimensions", "product"]]) {
    ...
    if (evidence) {
      ...
      if (english && !matchesCompleteListingDimensionEvidence(english, evidence)) {
        errors.push(`${field} must reproduce every complete source dimension tuple`);
      }
      if (chinese && !matchesCompleteListingDimensionEvidence(chinese, evidence)) {
