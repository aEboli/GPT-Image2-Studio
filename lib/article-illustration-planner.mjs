...
function countSourceParagraphs(content) {
...
function buildPlanningPrompt({ bundle, contentType, stylePreset }) {
  ...
  const sourceParagraphCount = countSourceParagraphs(bundle.content);

  return [
    "Read the complete article bundle before planning so consecutive frames stay coherent. Then cover every visual beat; do not collapse the story into a handful of summary images.",
    typeInstruction,
    "Create an article illustration plan as a dense animation-keyframe storyboard, not a sparse magazine edit.",
    `The source has about ${sourceParagraphCount} natural paragraph clusters. Produce at least one finished storyboard illustration for every paragraph or distinct visual beat, and extra consecutive frames whenever action, emotion, dialogue, camera angle, or location changes.`,
    "Treat adjacent storyboards as playable animation frames: each image must show the next moment so the full set can be read in order as one continuous sequence. Do not economize, merge multiple beats into one image, or apply a maximum illustration count.",
    "Set recommendedImageCount to the actual finished-frame count you produce. Prefer more frames over fewer whenever a scene can be split into consecutive shots.",
