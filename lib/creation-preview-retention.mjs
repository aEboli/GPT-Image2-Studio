...
// replaces its preview with the final image. It also protects a completed saved
// item from an older manifest or a late in-flight update that has no saved asset.

export function hasStoredCreationAsset(item = {}) {
...
export function hasCompletedCreationAsset(item = {}) {
  return String(item?.status || "").trim().toLowerCase() === "completed" && hasStoredCreationAsset(item);
}

export function shouldRetainCompletedCreationItem(previousItem = null, incomingItem = {}) {
  return hasCompletedCreationAsset(previousItem) && !hasStoredCreationAsset(incomingItem);
}

export function mergeCreationItemStreamUpdate(previousItem = {}, incomingItem = {}) {
  if (shouldRetainCompletedCreationItem(previousItem, incomingItem)) {
    return { ...previousItem };
  }
...
export function mergeCreationItemPreview(incomingItem = {}, previousItem = null) {
  ...
  if (hasStoredCreationAsset(incomingItem) || hasDisplayableImage(incomingItem)) {
    return incomingItem;
  }
