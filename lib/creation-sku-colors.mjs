...
export const CREATION_SKU_COLOR_NAME_OPTIONS = [
  { zh: "银灰色", en: "silver gray", tokens: ["银灰色", "银灰", "灰银色", "灰银", "silver gray", "silver grey", "gray silver", "grey silver"] },
...
const CJK_COLOR_CONTEXT_SUFFIX_PATTERN = /^(?:色)?(?:款|配色|色系|外观|外壳|壳体|主体|机身|框架|镜框|镜片|绑带|带子|表带|鞋面|鞋|衣|服|裤|裙|帽|背包|包|手袋|箱|盒|瓶|杯|车|路亚|鱼饵|拟饵|产品|商品|部件|组件|材质|涂层|表面|结构|纹理|鳞纹|鳞片|斑点|条纹|条带|尾部|鳃部|背部|腹部|鱼身|颜色|配色|色款|色系|版本|型号|选项|可选|扣|扣件)/u;
const CJK_SCRIPT_CHARACTER_PATTERN = /[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Hangul}]/u;
const CJK_COLOR_LIST_DELIMITER_PATTERN = /(?:[、,，/]|\s+(?:and|or|及|和|与)\s*)$/iu;
const CJK_COLOR_LIST_CONNECTOR_PATTERN = /^(?:和|及|与|或|以及|并且|并|加|配|带)?(?:多)?色/u;
...
function isContextSafeCjkColorMatch(source, start, end) {
  ...
  const hasListDelimiterBefore = CJK_COLOR_LIST_DELIMITER_PATTERN.test(before);
  const hasListDelimiterAfter = /^(?:[、,，/]|\s+(?:and|or|及|和|与)\s*)/iu.test(after);
  const hasColorListConnectorAfter = CJK_COLOR_LIST_CONNECTOR_PATTERN.test(after);
  return (
    CJK_COLOR_CONTEXT_PREFIX_PATTERN.test(before) ||
    CJK_COLOR_CONTEXT_SUFFIX_PATTERN.test(after) ||
    hasListDelimiterBefore ||
    hasListDelimiterAfter ||
    hasColorListConnectorAfter
  );
}
...
function getCreationSkuColorMatches(value) {
...
export function extractCreationSkuColorNames(value) {
  return getCreationSkuColorMatches(value).map((entry) => entry.colorName);
}
...
function localizeCreationSkuColorLabel(label = "", targetValue = "") {
  const colorMatches = getCreationSkuColorMatches(label);
  const normalizedLabel = colorMatches.map((entry) => entry.colorName).join(" ");
  if (!normalizedLabel || (targetValue !== "en" && targetValue !== "zh-CN")) {
    return normalizedLabel;
  }
  return colorMatches
    .map(({ option }) => targetValue === "en" ? option.en : option.zh)
    .map(normalizeCreationSkuVisibleColorLabel)
    .filter(Boolean)
    .join(" ");
}

export function getCreationSkuColorNames(skuSubject = {}, targetLanguage = {}) {
  const subject = skuSubject && typeof skuSubject === "object" ? skuSubject : {};
  const targetValue = cleanString(targetLanguage?.value || targetLanguage);
  const structuredColorSource = getCreationSkuStructuredColorLabels(subject);
  if (structuredColorSource !== null) {
    return normalizeCreationSkuColorLabels(structuredColorSource)
      .map((label) => localizeCreationSkuColorLabel(label, targetValue));
  }

  const seen = new Set();
  const explicitColorNames = [
    subject.colorName,
    subject.color_name,
    subject.color,
    subject.colour,
  ]
    .flatMap(splitCreationSkuColorLabels)
    .map((label) => localizeCreationSkuColorLabel(label, targetValue))
    .filter((label) => {
  ...
  const sourceText = [
    ...
    .filter(Boolean)
    .join(" ");
  return getCreationSkuColorMatches(sourceText)
    .map(({ option }) => normalizeCreationSkuVisibleColorLabel(targetValue === "en" ? option.en : option.zh))
    .filter(Boolean);
}

function compactCreationSkuChineseColorLabel(label = "") {
  const tokens = cleanString(label).split(/\s+/u).filter(Boolean);
  if (tokens.length < 2 || !tokens.every((token) => /色$/u.test(token))) {
    return cleanString(label);
  }
  return tokens
    .map((token, index) => index === tokens.length - 1 ? token : token.slice(0, -1))
    .join("");
}

function getCreationSkuSubjectUnitCount(skuSubject = {}) {
  const count = Number.parseInt(
    cleanString(skuSubject.subjectUnitCount ?? skuSubject.subject_unit_count),
    10,
  );
  return Number.isFinite(count) && count > 1 ? count : 0;
}

export function formatCreationSkuItemColorNames(skuSubject = {}, targetLanguage = {}) {
  const subject = skuSubject && typeof skuSubject === "object" ? skuSubject : {};
  const targetValue = cleanString(targetLanguage?.value || targetLanguage);
  const colorNames = getCreationSkuColorNames(subject, targetLanguage);
  const subjectUnitCount = getCreationSkuSubjectUnitCount(subject);
  const keepsLabelBoundaries =
    getCreationSkuStructuredColorLabels(subject) !== null ||
    (subjectUnitCount > 1 && colorNames.length === subjectUnitCount);
  const colorLabels = keepsLabelBoundaries ? colorNames : [colorNames.join(" ")];
  return colorLabels
    .map((label) => targetValue === "zh-CN" ? compactCreationSkuChineseColorLabel(label) : cleanString(label))
    .filter(Boolean)
    .join(" / ");
}
