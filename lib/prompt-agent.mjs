...
const CREATION_REFERENCE_ANALYSIS_SKU_INSTRUCTION = [
  ...
  "For every sku_subject, return sku_subjects[].color_names with exactly one concise English characteristic-color label per complete visible product unit, in the same left-to-right or top-to-bottom order as those units. Each unit label must include all clearly visible characteristic colors of the physical sellable product, including shared neutral component colors such as a black strap or gray frame. Keep all colors for one product unit together inside one array string, and add a short component name only when needed to make the color unambiguous, for example brown, black, silver lenses. Do not split one unit's characteristic colors into multiple color_names items. Exclude the white/transparent background, shadows, highlights, environmental reflections, source-card text, and uncertain colors. If safe characteristic-color labels cannot be supplied for every complete unit in the group, return an empty color_names array and never guess.",
      ...
      items: {
        ...
        properties: {
          ...
          color_names: {
            ...
            maxItems: 20,
            items: { type: "string" },
            description: "Exactly one concise English characteristic-color label per complete visible product unit, ordered left-to-right or top-to-bottom, or an empty array when safe labels are unavailable for the complete group. Include all clearly visible characteristic colors of the physical product, including shared neutral-colored physical components, and add a short component name when needed. Preserve each complete unit label as one array item even when it contains multiple colors. Exclude backgrounds, shadows, highlights, environmental reflections, overlay text, and uncertain colors.",
          },
          note: {
            type: "string",
  ...
  return Array.isArray(value)
    ? value
        .map((item, index) => {
          ...
          const colorNames = normalizeStringArray(Array.isArray(rawColorNames) ? rawColorNames : rawColorNames ? [rawColorNames] : []);
