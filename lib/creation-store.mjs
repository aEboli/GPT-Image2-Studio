...
function normalizeResolutionRatio(value, fallback = "1:1") {
  return resolveAspectRatioOption(cleanString(value) || fallback).value;
}

function isHistoricalAutomaticSize(value) {
  return cleanString(value).toLowerCase() === "auto";
}

function getResolutionMigrationContext(value = {}, parent = {}) {
  const ratio = normalizeResolutionRatio(value.ratio, parent.ratio || "1:1");
  const imageRoute = cleanString(value.imageRoute || value.generationRoute || parent.imageRoute || parent.generationRoute);
  const defaultSize = imageRoute === "c"
    ? getDefaultModelProtocolImageSize()
    : getDefaultGenerationSize(ratio);
  const concreteSize = [value.effectiveSize, value.size, value.requestedSize]
    .map(cleanString)
    .find((candidate) => candidate && !isHistoricalAutomaticSize(candidate));
  return {
    ratio,
    imageRoute,
    defaultSize,
    fallbackSize: concreteSize || defaultSize,
    fallbackTier: "1K",
  };
}

function migrateResolutionValue(value, fallback) {
  const normalized = cleanString(value);
  return isHistoricalAutomaticSize(normalized) ? fallback : normalized;
}

function normalizeResolutionMetadataTree(value, parent = {}) {
  if (Array.isArray(value)) {
    return value.map((entry) => normalizeResolutionMetadataTree(entry, parent));
  }
  if (!value || typeof value !== "object") {
    return value;
  }

  const context = getResolutionMigrationContext(value, parent);
  const normalized = { ...value };
  for (const field of ["size", "requestedSize", "effectiveSize"]) {
    if (Object.prototype.hasOwnProperty.call(normalized, field)) {
      normalized[field] = migrateResolutionValue(normalized[field], context.fallbackSize);
    }
  }
  if (Object.prototype.hasOwnProperty.call(normalized, "resolutionTier")) {
    normalized.resolutionTier = migrateResolutionValue(normalized.resolutionTier, context.fallbackTier);
  }

  for (const [key, entry] of Object.entries(normalized)) {
    if (["size", "requestedSize", "effectiveSize", "resolutionTier", "actualSize"].includes(key)) {
      continue;
    }
    if (Array.isArray(entry) || (entry && typeof entry === "object")) {
      normalized[key] = normalizeResolutionMetadataTree(entry, context);
    }
  }
  return normalized;
}

function normalizeCreationItem(item = {}, publicBasePath, parent = {}) {
  ...
  const resolutionContext = getResolutionMigrationContext(item, parent);
  const normalized = {
    ...
    generationAttemptCount: Number.isFinite(Number(item.generationAttemptCount))
      ...
      : 0,
    ...(Object.prototype.hasOwnProperty.call(item, "originalResponseRecovery")
      ? { originalResponseRecovery: cleanString(item.originalResponseRecovery) }
      : {}),
    ...(Object.prototype.hasOwnProperty.call(item, "originalResponseRecoveryReason")
      ? { originalResponseRecoveryReason: cleanString(item.originalResponseRecoveryReason) }
      : {}),
    ...(Object.prototype.hasOwnProperty.call(item, "originalResponseStatus")
      ? { originalResponseStatus: cleanString(item.originalResponseStatus) }
      : {}),
    ...(Object.prototype.hasOwnProperty.call(item, "originalResponseCheckedAt")
      ? { originalResponseCheckedAt: cleanString(item.originalResponseCheckedAt) }
      : {}),
    ...(Object.prototype.hasOwnProperty.call(item, "originalResponseAutoRetryBlocked")
      ? { originalResponseAutoRetryBlocked: normalizeDefaultEnabledBoolean(item.originalResponseAutoRetryBlocked) }
      : {}),
    size: migrateResolutionValue(item.size, resolutionContext.fallbackSize),
    requestedSize: migrateResolutionValue(item.requestedSize, resolutionContext.fallbackSize),
    effectiveSize: migrateResolutionValue(item.effectiveSize, resolutionContext.fallbackSize),
  ...
  for (const key of [
    ...
    if (item[key] !== undefined) {
      ...
      if (key === "resolutionTier") {
        normalized[key] = migrateResolutionValue(normalized[key], resolutionContext.fallbackTier);
      }
    }
  }
  ...
  if (Object.prototype.hasOwnProperty.call(normalized, "quality")) {
    normalized.quality = normalizeStoredImageQuality(normalized.quality, {
...
export function normalizeCreationSetManifest(manifest = {}, { publicBasePath = "/output" } = {}) {
  ...
  const manifestResolutionContext = getResolutionMigrationContext(manifest);
  const items = Array.isArray(manifest.items)
    ? manifest.items.map((item) => normalizeCreationItem(item, publicBasePath, manifestResolutionContext)).sort((a, b) => a.slotIndex - b.slotIndex)
    : [];
  ...
  const effectivePlan = manifest.effectivePlan && typeof manifest.effectivePlan === "object"
    ? normalizeResolutionMetadataTree(manifest.effectivePlan, manifestResolutionContext)
    : null;
  ...
  const dimensionSpecGroups = normalizeCreationDimensionGroups(
    manifest.dimensionSpecGroups ?? manifest.dimension_spec_groups,
  );
  ...
  return {
    ...
    platformSetOverrides: manifest.platformSetOverrides && typeof manifest.platformSetOverrides === "object"
      ? normalizeResolutionMetadataTree(manifest.platformSetOverrides, manifestResolutionContext)
      : {},
    platformItemOverrides: Array.isArray(manifest.platformItemOverrides)
      ? normalizeResolutionMetadataTree(manifest.platformItemOverrides, manifestResolutionContext)
      : [],
    effectivePlan,
...
export function createCreationSetStore({ outputDir, publicBasePath = "/output" }) {
  ...
  async function recoverCreationItemSnapshot(item) {
    ...
    if (Object.prototype.hasOwnProperty.call(recovered, "quality")) {
      recovered.quality = normalizeStoredImageQuality(recovered.quality, {
        imageRoute: recovered.imageRoute,
        imageModel: recovered.imageModel,
      });
    }
    ...
    return normalizeResolutionMetadataTree(recovered, {
      ratio: recovered.ratio || item.ratio,
      imageRoute: recovered.imageRoute || item.imageRoute,
    });
  }
