...
export const CREATION_REFERENCE_ROLE_OPTIONS = [
  ...
  },
  {
    ...
    promptInstruction: "Use it to preserve material texture, finish, seams, surface detail, visible external structure, feature callouts, and annotated detail accuracy, as supporting evidence rather than a sellable product subject.",
  },
  {
    value: "dimensions",
    label: "尺寸规格",
    promptLabel: "dimensions and specifications",
    promptInstruction: "Use it for size charts, measurements, capacity, weight, compatibility, and specification values, as a fact source rather than a sellable product subject.",
  },
  {
    value: "usage",
    label: "使用说明",
    promptLabel: "usage instructions",
    promptInstruction: "Use it to read setup, operation, charging, connection, assembly, safety notes, and instruction callouts as source facts, rather than as a sellable product subject.",
  },
  {
    value: "scene",
    label: "使用场景",
...
function buildCreationDimensionPromptInstruction({
  ...
  if (cleanString(role) === "spec-table") {
    ...
    const dualUnitLock =
      dimensionUnitMode.value === "both"
        ...
        : "";
    return [
      `Visible key specifications, the complete set for this image: ${dimensionSpecSummary}.`,
  ...
  if (dimensionSpecSummary) {
    ...
    return `${heading}: ${dimensionSpecSummary}. ${dimensionUnitMode.promptInstruction} ${dualUnitLock} ${buildCreationExactNumericValueLock(dimensionSpecSummary)} ${mandatory}`;
  }
...
function buildCreationPrimarySubjectLock(referenceImageRoles = []) {
  ...
  return [
    `Primary subject: ${primarySubject.filename} is the main visual product for every non-SKU image in this set, keeping its silhouette, proportions, colorway, materials, logos, markings, hardware, front and back structure, straps or handles, seams, and visible feature placement.`,
    hasOtherProductSubjects
      ...
      : "",
    "Supporting references contribute only their assigned role content: background, scene, dimensions, usage, material, or package.",
  ]
    .filter(Boolean)
    .join(" ");
}
...
function buildCreationReferenceGuidance(referenceImageRoles = []) {
  ...
  return `Reference image roles: ${roleLines} ${buildCreationPrimarySubjectLock(referenceImageRoles)} Each reference influences only its listed role. Usage sources serve as selling-point evidence.`;
}
...
function buildCreationLogoGuidance(logoOptions = {}) {
  ...
  return [
    `Logo ${logo.filename}: place it at the ${logo.promptPosition} with clean safe margins, legible and proportional as the only added brand mark.`,
...
function buildCreationTargetLanguageTextGuidance(targetLanguage) {
  ...
  return `Added canvas text uses ${targetLabel}, unique to this image, with brand names, model names, numbers, and units exact.`;
}
