...
import {
  CREATION_SKU_COLOR_NAME_OPTIONS,
  extractCreationSkuColorNames,
  findCreationSkuColorTokenMatches,
...
function getCreationSkuStructuredColorLabels(skuSubject = {}) {
  if (Array.isArray(skuSubject.colorNames)) {
    return skuSubject.colorNames;
  }
  if (Array.isArray(skuSubject.color_names)) {
    return skuSubject.color_names;
  }
  return null;
}

function normalizeCreationSkuVisibleColorLabel(value = "") {
  return normalizeCreationSkuColorLabels([value])[0] || "";
}

function localizeCreationSkuExplicitColorName(label = "", targetValue = "") {
  const normalizedLabel = normalizeCreationSkuVisibleColorLabel(label);
  if (!normalizedLabel || (targetValue !== "en" && targetValue !== "zh-CN")) {
    return normalizedLabel;
  }
  return extractCreationSkuColorNames(normalizedLabel)
    .map((colorName) => {
      const matchedOption = CREATION_SKU_COLOR_NAME_OPTIONS.find((option) =>
        option.tokens.some((token) => cleanString(token).toLowerCase() === colorName.toLowerCase()),
      );
      return matchedOption
        ? normalizeCreationSkuVisibleColorLabel(targetValue === "en" ? matchedOption.en : matchedOption.zh)
        : colorName;
    })
    .filter(Boolean)
    .join(" ");
...
function getCreationSkuExplicitColorNames(skuSubject = {}, targetLanguage = {}) {
  ...
  const structuredColorSource = getCreationSkuStructuredColorLabels(skuSubject);
  if (structuredColorSource !== null) {
    const structuredColorNames = normalizeCreationSkuColorLabels(structuredColorSource);
    return structuredColorNames.map((label) => localizeCreationSkuExplicitColorName(label, targetValue));
  }
  const seen = new Set();
  return [
...
function getCreationSkuColorTokenIndex(sourceText = "", token = "") {
  return findCreationSkuColorTokenMatches(cleanString(sourceText), cleanString(token))[0]?.start ?? -1;
}
...
function inferCreationSkuColorNames(skuSubject = {}, targetLanguage = {}) {
  ...
  if (getCreationSkuStructuredColorLabels(skuSubject) !== null || explicitColorNames.length > 0) {
  ...
  return CREATION_SKU_COLOR_NAME_OPTIONS
    ...
    .map(({ option }) => normalizeCreationSkuVisibleColorLabel(targetValue === "en" ? option.en : option.zh))
    .filter(Boolean);
}
  ...
  const carouselItems = plannedRoles.map((role, index) => {
    ...
    const roleGuidance = platformPolicyDominant
      ...
      : [
          ...
          platformNativeSlot ? "" : getCreationRoleIntentInstruction(role.role),
          ...
          buildCreationVisualLanguageGuidance(visualLanguage),
          platformNativeSlot && !["spec-table", "accessory-gift"].includes(role.role)
            ? ""
