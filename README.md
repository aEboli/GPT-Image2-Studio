...
- **电商套图**：按平台、类目、商品事实、受众和 SKU 生成推荐轮播计划，支持逐图调整、Logo、补图队列和 Listing 草稿；手动轮播最多选择 18 个图片角色。
...
从商品事实、参考图和目标平台生成套图计划，再逐图检查、调整和批量执行。
...
| 电商套图 | `#creation`，创作 -> 套图模式 | 商品事实、最多 15 张产品参考图、平台、类目、SKU、语言、Logo 与计划覆盖项 | 智能识别参考图角色，按 19 种平台画像生成轮播计划，可逐图修改类型、比例、档位、语言和提示词，再并发生成 | 套图、队列、失败项补齐、逐图提示词编辑、Logo 批处理、Listing 草稿及套图记录 |
...
| --- | --- | --- | --- | --- |
| 瀑布画廊 | `#gallery`，资产 -> 瀑布画廊 | 工作流、日期、关键词和列数 | 合并服务端记录与浏览器缓存，按筛选条件构建瀑布流 | 灯箱缩放/平移、前后切换、下载、复制路径、查看完整参数和删除资产 |
| 文章插图记录 | `#article-record`，资产 -> 文章插图记录 | 记录搜索、列数、文章插图集 | 按文章集加载参考卡、正文插图、标题和说明文字 | 继续创作、复制说明、逐图查看/下载和重新生成 |
| 套图记录 | `#creation-record`，资产 -> 套图记录 | 商品/平台搜索、已保存套图 | 恢复生成快照、有效计划、进度、Listing 和逐图结果 | 导出 TXT/JSON、补齐缺失项、继续编辑提示词、查看灯箱和下载 |
| 写真记录 | `#portrait-record`，资产 -> 写真记录 | 写真集搜索与已保存计划 | 恢复人物、地点、风格、景别、动作和生成进度 | 导出 JSON、重试失败项、查看/下载结果和回到写真模式继续创作 |
| PPT 记录 | `#ppt-record`，资产 -> PPT记录 | 已生成演示文稿列表 | 读取大纲、页面缩略图、导出文件和完成状态 | 逐页预览、下载 PPTX、继续补页或进入单页编辑器 |
...
| 电商套图 | 产品参考图最多 15 张；Logo 最多 1 张；上传图加 Logo 的源图最多 15 张 | 平台推荐轮播为 6 到 9 张；逐图高级编辑中的兼容角色选择器最多 18 个，`18` 仅是手动轮播上限，不是所有平台推荐值 |
| 电商 SKU | SKU 组合数量最多 20 | 冻结计划最多 64 项、序列化后最多 750,000 字节，属于内部提交保护上限 |
...
平台表来自应用内版本 `2026-07-14.1` 的策略画像。推荐轮播数只统计平台画像的默认轮播槽位，不含按 SKU 追加的图片、信息图重构项或用户手动新增项。

| 平台 | 推荐轮播 | 分辨率档位 | 目标语言 | 证据级别 |
| --- | ---: | --- | --- | --- |
| 通用电商 | 8 | `1.5K` | English | baseline |
...
| 电商套图 | 默认 PNG；平台画像决定默认比例、档位、语言和图片类型 | 用户逐图覆盖后以提交时冻结的有效计划为准；手动角色上限 18 不覆盖平台建议 |
