...
const DEFAULT_GALLERY_LAYOUT_SECTION_GROUP_SIZE = 2;
const DEFAULT_GALLERY_MAX_PAGE_ROWS = 3;
const DEFAULT_GALLERY_MIN_SECTION_COLUMNS = 4;
const DEFAULT_GALLERY_MAX_SECTION_COLUMNS = 18;
...
function getGallerySectionItemCount(section) {
  return Array.isArray(section?.items) ? section.items.length : 0;
}

export function getGallerySectionLayout(sections, options = {}) {
  const sourceSections = (Array.isArray(sections) ? sections : []).filter(
    (section) => getGallerySectionItemCount(section) > 0,
  );
  if (sourceSections.length === 0) {
    return [];
  }

  const maxRows = Math.max(sourceSections.length, Number(options.maxRows) || DEFAULT_GALLERY_MAX_PAGE_ROWS);
  const minColumns = Math.max(1, Number(options.minColumns) || DEFAULT_GALLERY_MIN_SECTION_COLUMNS);
  const maxColumns = Math.max(minColumns, Number(options.maxColumns) || DEFAULT_GALLERY_MAX_SECTION_COLUMNS);
  const rowCounts = sourceSections.map(() => 1);

  for (let remainingRows = maxRows - sourceSections.length; remainingRows > 0; remainingRows -= 1) {
    let targetIndex = 0;
    let highestDensity = -1;
    sourceSections.forEach((section, index) => {
      const density = getGallerySectionItemCount(section) / rowCounts[index];
      if (density > highestDensity) {
        targetIndex = index;
        highestDensity = density;
      }
    });
    rowCounts[targetIndex] += 1;
  }

  return sourceSections.map((section, index) => {
    const itemCount = getGallerySectionItemCount(section);
    const allocatedRows = rowCounts[index];
    const columnCount = Math.max(minColumns, Math.ceil(itemCount / allocatedRows));
    const resolvedColumnCount = Math.min(maxColumns, columnCount);
    return {
      key: section.key,
      rowCount: Math.max(1, Math.ceil(itemCount / resolvedColumnCount)),
      columnCount: resolvedColumnCount,
    };
  });
}

export function getGalleryHistorySectionLayouts(sections, options = {}) {
  const sourceSections = (Array.isArray(sections) ? sections : []).filter(
    (section) => getGallerySectionItemCount(section) > 0,
  );
  const groupSize = Math.max(1, Math.floor(Number(options.groupSize) || DEFAULT_GALLERY_LAYOUT_SECTION_GROUP_SIZE));
  const layouts = [];
  for (let index = 0; index < sourceSections.length; index += groupSize) {
    layouts.push(...getGallerySectionLayout(sourceSections.slice(index, index + groupSize), options));
  }
  return layouts;
}
