...
function normalizePromptAgentJson(value) {
  ...
  const audienceStrategy = rawAudienceStrategy && typeof rawAudienceStrategy === "object" && !Array.isArray(rawAudienceStrategy)
    ? {
        target_audience: String(rawAudienceStrategy.target_audience || rawAudienceStrategy.targetAudience || "").trim(),
        purchase_motivations: normalizeStringArray(rawAudienceStrategy.purchase_motivations || rawAudienceStrategy.purchaseMotivations).slice(0, 5),
        purchase_objections: normalizeStringArray(rawAudienceStrategy.purchase_objections || rawAudienceStrategy.purchaseObjections).slice(0, 5),
        desired_outcome: String(rawAudienceStrategy.desired_outcome || rawAudienceStrategy.desiredOutcome || "").trim(),
        evidence_basis: normalizeStringArray(rawAudienceStrategy.evidence_basis || rawAudienceStrategy.evidenceBasis).slice(0, 5),
        confidence: ["low", "medium", "high"].includes(String(rawAudienceStrategy.confidence || "").trim().toLowerCase())
          ? String(rawAudienceStrategy.confidence).trim().toLowerCase()
          : "low",
