...
function resolveListingRequestSource(source = {}) {
  const legacyContract = !hasExplicitListingTarget(source)
  ...
  return {
    source: {
      ...source,
      ...(legacyContract ? {} : { schemaVersion: "2", forceV2: true }),
...
function buildPlatformListingPrompt(source = {}, policy = {}, validationErrors = []) {
  ...
  const amazonGuidance = platformId === "amazon"
    ? [
      ...
      "Use quantity, brand, model, variant, or other identifying attributes only when supplied by Source JSON; do not force quantity to the first position.",
    ]
    : [];
  return [
    ...
    ...amazonGuidance,
    `Locale and units: write public fields in ${locale}. ${selectedUnitModeInstruction(source)}`,
    `Cross-category playbook order: ${(policy.conversionOrder || []).join(" > ")}.`,
    "Cover product identity and search intent, fact-supported benefits, real use, buyer objections, size/fit/variants/package clarity, and missing-evidence disclosure where relevant. Do not invent category facts.",
    Array.isArray(source.skuSubjects) && source.skuSubjects.length > 0
      ? "Treat supplied SKU subjects as variants/options within this single parent listing; do not create separate listings."
      : "",
    groupedSubjectPromptInstruction(source),
    "Return only the V2 JSON object required by the supplied strict schema. Use highlights for the policy-specific bullets/features/benefits surface and searchTerms for the policy-specific search surface. sellingPoints, buyerObjections, and keywordBuckets are internal planning fields unless the resolved policy explicitly publishes them.",
...
function makeReviewableInputOnlyFallback(source = {}, policy = {}, validationErrors = []) {
  const details = (Array.isArray(validationErrors) ? validationErrors : [])
    .map(truncateField)
    .filter(Boolean);
  return normalizeCreationListingDraft({
    ...
    language: source.language || policy.language || policy.locale || policy.defaultLocale || "en-US",
    title: "",
    sellingPoints: [],
    buyerObjections: [],
    highlights: [],
    ...
    evidence: [],
    missingInfo: ["Upstream output remained invalid after two attempts; complete publishable fields from verified product inputs."],
    warnings: [
      "This input-only placeholder requires manual review and is not ready to publish.",
      ...details,
    ],
...
function buildLegacyAmazonFallbackCopyProfile(skuName = "Product", { quantitySize = "", variants = [], groupedSubjectDetail = "" } = {}) {
...
export function makeMockCreationListingDraft(source = {}) {
  ...
  if (source.forceV2 === true) {
    const suppliedTitle = truncateField(source.productName || source.skuTitle || "");
    const suppliedHighlights = (Array.isArray(source.sellingPoints) ? source.sellingPoints : [])
      .map(truncateField)
      .filter(Boolean);
    const suppliedDescription = truncateField(source.productDescription || source.skuNote || "");
    return normalizeCreationListingDraft({
      schemaVersion: "2",
      platformId,
      platformLabel: source.platformLabel || policy.platformLabel || policy.label,
      marketplace: source.marketplace || policy.marketplaceId,
      listingPolicyVersion: source.listingPolicyVersion || policy.listingPolicyVersion || policy.policyVersion,
      language: source.language || policy.language || policy.locale || policy.defaultLocale,
      title: suppliedTitle,
      sellingPoints: suppliedHighlights,
      buyerObjections: [],
      highlights: suppliedHighlights,
      description: suppliedDescription,
      searchTerms: suppliedTitle ? [suppliedTitle] : [],
      keywordBuckets: {
        exact: suppliedTitle ? [suppliedTitle] : [],
        longTail: [],
        traffic: [],
        descriptive: [],
      },
      evidence: suppliedTitle || suppliedDescription || suppliedHighlights.length > 0 ? ["product-input"] : [],
      missingInfo: ["Mock mode did not call the upstream Listing model; complete and review each publishable field."],
      warnings: ["Mock output is an input-only placeholder and is not ready to publish."],
      evidenceMode: "input-only",
      status: "needs-review",
    }, source);
  }
  ...
  const groupedSubjectDetail = groupedSubjectDescriptionLine(source);
  const profile = buildLegacyAmazonFallbackCopyProfile(skuName, { quantitySize, variants, groupedSubjectDetail });
...
export async function requestCreationListingDraft({
  ...
  if (!requestContext.legacyContract) {
    return makeReviewableInputOnlyFallback(requestSource, requestContext.policy, validationErrors);
  }
