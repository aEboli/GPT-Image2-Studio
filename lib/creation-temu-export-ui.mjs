import { resolveDisabledShakeTarget } from "./disabled-shake.mjs";
...
function getTemuExportDomRefs(documentRef) {
  return {
    ...
    recordFeedback: documentRef.querySelector("#creationRecordActionFeedback"),
...
export function createCreationTemuExportController({
  ...
  fetchImpl = fetch,
  urlApi = URL,
  // 注入后，点击「temuexcel导出工作台」打开上品工作台覆盖层；
  // 不注入则退回原行为（直接打开批量导出对话框）。
  openWorkbench = null,
} = {}) {
  ...
  // 撤回上一次因禁用而播报的原因：只在反馈区仍显示那句话时清空，
  // 避免盖掉其它操作之后写入的新反馈。
  function clearBlockedNotice() {
    if (!blockedNotice) return;
    const announced = blockedNotice;
    blockedNotice = "";
    if (refs.recordFeedback?.textContent === announced) setRecordFeedback?.("");
  }

  function syncControls(blocked, checkedCount) {
    startBlocked = Boolean(blocked) || Boolean(isMutationBusy?.());
    if (refs.exportButton) {
      refs.exportButton.disabled = startBlocked;
      refs.exportButton.textContent = isBusy()
        ? phase === "preflight" ? "正在预检..." : "正在导出..."
        : "temuexcel导出工作台";
    }
    if (refs.exportButton && !refs.exportButton.disabled) clearBlockedNotice();
  ...
  // 忙碌状态和批量导出的勾选要求分别建模：主入口可以空批次进工作台，
  // 覆盖层内的批量快速导出仍只能处理明确勾选的记录。
  function getOpenBlockReason({ requireCheckedSets = true } = {}) {
    if (isBusy() || startBlocked || isMutationBusy?.()) {
      return "当前记录正在生成、刷新或删除，请完成后再打开 Temu 导出。";
    }
    if (requireCheckedSets && getCheckedSets().length === 0) {
      return "请先勾选需要导出的套图记录。";
    }
    return "";
  }

  function passesOpenGuards(options) {
    const reason = getOpenBlockReason(options);
    if (!reason) return true;
    setRecordFeedback?.(reason, "error");
    return false;
  }

  /* 禁用的「temuexcel导出工作台」设了 pointer-events: none，click 与 pointerdown 都到不了
     它身上，事件会改指向祖先容器。这里复用全局抖动模块的同一套命中判定，保证抖动的控件
     与播报原因的控件永远是同一个；只取原因、不调用 open()，被禁用的动作绝不会因此执行。 */
  function handleDisabledExportPress(event) {
    if (!refs.exportButton?.disabled) return;
    if (event?.button !== undefined && event.button !== 0) return;
    if (resolveDisabledShakeTarget(event) !== refs.exportButton) return;
    const reason = getOpenBlockReason();
    if (!reason) return;
    blockedNotice = reason;
    setRecordFeedback?.(reason, "error");
  }

  // 主入口始终无预选地打开工作台，避免自动跳到 Studio 导入对话框；
  // 未注入工作台时才退回原有的勾选批量导出行为。
  function open() {
    if (typeof openWorkbench === "function") {
      if (!passesOpenGuards({ requireCheckedSets: false })) return;
      openWorkbench([]);
      return;
    }
    openExportDialog();
  ...
  // 原 open 的主体：批量快速导出对话框。现由覆盖层内第二个标签调用。
  function openExportDialog() {
    if (!passesOpenGuards()) return;
  ...
  function bind() {
    ...
    documentRef.addEventListener?.("pointerdown", handleDisabledExportPress, true);
  }

  bind();
  return { close, getCheckedSetIds, open, openExportDialog, runPreflight, submit, syncControls };
}
