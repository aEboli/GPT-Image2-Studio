...
function normalizeArticleItemSize(item = {}) {
  const rawSize = cleanString(item.size);
  if (!rawSize) {
    return "";
  }

  const imageRoute = cleanString(item.imageRoute || item.generationRoute).toLowerCase();
  ...
  return imageRoute === "c"
    ...
    : getDefaultGenerationSize(cleanString(item.ratio) || "3:2");
}
...
function normalizeArticleItem(item = {}, publicBasePath) {
  ...
  return {
    ...
    size: normalizeArticleItemSize(item),
...
export function createArticleIllustrationSetStore({ outputDir, publicBasePath = "/output" }) {
  ...
  const deleteGuard = createRecordDirectoryDeleteGuard({
    outputDir,
    manifestsDir,
    markerPattern: /^\d{4}-\d{2}-\d{2}-article$/u,
  });
  ...
  async function deleteManifests(setIds) {
    const normalizedSetIds = normalizeAssetRecordDeleteIds(setIds, { recordLabel: "文章插图记录" });
