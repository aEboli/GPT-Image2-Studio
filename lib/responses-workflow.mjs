...
export function isMissingMultipartImageFieldError({ status, body } = {}) {
...
export async function requestDirectImageGeneration({
  ...
  const usableReferenceImages = filterUsableImageInputs(referenceImages);
  const buildImageEditBody = (imageFieldName) =>
    createImageEditFormData({
      prompt: buildImageEditPromptWithReferenceLabels(prompt, referenceImageLabels),
      sourceImages: usableReferenceImages,
      size,
      quality,
      format,
      imageModel,
  ...
  if (!response.ok) {
    const errorBody = await readResponseText(response, requestSignal);
    const canRetryWithSingularField =
      isMultipartRequest &&
      usableReferenceImages.length > 1 &&
      isMissingMultipartImageFieldError({ status: response.status, body: errorBody });

    if (!canRetryWithSingularField) {
      throw new Error(
        formatHttpErrorMessage({
    ...
    await emitEvent(onEvent, {
      type: "status",
      stage: "connecting",
      message: `上游未识别 ${MULTI_IMAGE_FIELD_NAME} 多图字段，正在改用重复 ${SINGLE_IMAGE_FIELD_NAME} 字段重试。`,
    });

    imageFieldFallbackUsed = true;
    response = await postRequest(await buildImageEditBody(SINGLE_IMAGE_FIELD_NAME));
