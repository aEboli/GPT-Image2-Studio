...
const ITEM_SUPPORTING_REFERENCE_ROLES = {
  ...
  benefit: ["material"],
  scene: ["scene"],
  "multi-angle": ["product", "material"],
  atmosphere: ["scene"],
  "product-detail": ["material"],
  "brand-story": ["material", "scene"],
...
export function appendCreationItemLogoReference(item = {}, referenceImages = [], logoImage = null) {
  const images = normalizeImages(referenceImages);
  if (cleanString(item?.role || item?.itemKind) === "infographic-rebuild") return images;
  if (!logoImage) return images;
  const logoPolicy = cleanString(item?.logoPolicy).toLowerCase();
  if (logoPolicy && logoPolicy !== "allow-supplied") return images;
