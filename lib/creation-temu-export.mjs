...
const REQUEST_KEYS = new Set(["mode", "setIds", "defaults", "cloudinary"]);
...
export function normalizeTemuExportRequest(payload = {}) {
  ...
  const mode = cleanString(request.mode) || "draft";
  if (!new Set(["strict", "draft"]).has(mode)) {
    throw new Error("Temu 导出 mode 只支持 strict 或 draft。");
  }
...
function isPrivateIpv6(hostname) {
  ...
  const mappedTail = normalized.match(/^::ffff:(.+)$/u)?.[1] || "";
  if (mappedTail) {
    if (isPrivateIpv4(mappedTail)) return true;
    const groups = mappedTail.split(":");
    if (groups.length === 2 && groups.every((group) => /^[0-9a-f]{1,4}$/u.test(group))) {
      const bytes = groups.flatMap((group) => {
        const value = Number.parseInt(group, 16);
        return [value >> 8, value & 0xff];
      });
      if (isPrivateIpv4(bytes.join("."))) return true;
    }
  }
...
export function createTemuExportPlan({ sets = [], defaults = {} } = {}) {
  ...
  for (const set of sets) {
    ...
    const allSharedItems = sortedItems
    ...
    const sharedItems = allSharedItems.slice(0, 10);
    const materialItem = allSharedItems.find((item) => cleanString(item.role) === "hero") || allSharedItems[0] || null;
