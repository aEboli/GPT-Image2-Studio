...
const LOADING_THUMBNAIL_STAGE_LABELS = {
  ...
  recovering_original: "回查原任务",
  waiting_original: "等待原任务",
  recovered_original: "已找回原图",
  original_failed: "原任务失败",
  recovery_unavailable: "原结果未知",
  saving: "保存中",
  error: "失败",
  failed: "失败",
};
...
export function getPreviewPlaceholderState({
  ...
  return {
    mode: "loading",
    waiting: isWaitingPreviewItem(item),
    loadingKey: String(item.id || item.taskId || item.filename || "").trim(),
...
export function getStablePreviewLoadingItems(items = []) {
  return getPreviewLoadingItemRecords(items)
    .sort((left, right) =>
      String(right.entry?.createdAt || "").localeCompare(String(left.entry?.createdAt || "")) || left.index - right.index,
    )
    .map(({ entry }) => entry);
}
...
export function isWaitingPreviewItem(item = {}) {
