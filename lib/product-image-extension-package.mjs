...
const EXPECTED_LAUNCHER_MATCHES = [
  "https://detail.1688.com/offer/*",
  ...AMAZON_EXTENSION_HOSTS.flatMap((host) => [
    `https://*.${host}/dp/*`,
    `https://*.${host}/*/dp/*`,
    `https://*.${host}/gp/product/*`,
    `https://*.${host}/gp/aw/d/*`,
  ]),
  "https://*.temu.com/*-g-*.html",
  "https://*.temu.com/goods.html*",
...
function validateManifest(manifest) {
  ...
  if (serialized.includes("<all_urls>") || permissions.has("tabs") || permissions.has("sidePanel") || !hasExactHostPermission || !hasExactLauncher) {
    throw new Error("商品图采集插件只能声明受支持商城的受限悬浮入口内容脚本。");
  }
}
...
export async function buildProductImageCollectorArchive({ rootDir = defaultRootDir } = {}) {
  ...
  return {
    ...
    files: [...files, "lib/product-image-import.mjs", "lib/product-image-platforms.mjs"],
  };
}
