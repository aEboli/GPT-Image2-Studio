...
function normalizeDimensionSpecStrings(value) {
  if (Array.isArray(value)) {
    return value.flatMap((entry) => {
      if (entry && typeof entry === "object" && !Array.isArray(entry)) {
        ...
        return normalizeDimensionSpecStrings(entry);
      }
    ...
    return Object.entries(value).flatMap(([key, item]) => {
      if (Array.isArray(item)) {
        return item.flatMap((nested) => {
          const normalized = cleanString(nested);
          return normalized ? [`${key}: ${normalized}`] : [];
        });
      }
      if (item && typeof item === "object") {
        return normalizeDimensionSpecStrings(item).map((nested) => `${key}: ${nested}`);
      }
...
function normalizeDimensionGroups(value) {
  let entries = value;
  ...
  if (!Array.isArray(entries) && entries && typeof entries === "object") {
    const descriptorKeys = new Set([
      "id", "key", "groupId", "group_id", "label", "name", "title", "variant", "variantLabel", "variant_label",
      "color", "colorName", "color_name", "size", "sizeLabel", "size_label", "referenceIndexes", "reference_indexes",
      "reference_indices", "indexes", "indices", "filenames", "filename", "referenceFilenames", "reference_filenames",
      "specs", "specifications", "dimensions", "dimensionSpecs", "dimension_specs", "sizeSpecs", "size_specs", "lines", "values", "facts", "note", "description",
    ]);
    entries = Object.keys(entries).some((key) => descriptorKeys.has(key))
      ? [entries]
      : Object.entries(entries).map(([label, specs]) => {
          if (specs && typeof specs === "object" && !Array.isArray(specs) && Object.keys(specs).some((key) => descriptorKeys.has(key))) {
            return { ...specs, label: cleanString(specs.label || specs.name || specs.title || label) };
          }
          return { label, specs };
        });
  }
  ...
  return entries
    .map((entry, index) => {
      ...
      const referenceIndexes = normalizeIndexArray(
        entry.referenceIndexes || entry.reference_indexes || entry.indexes || entry.indices,
      );
      const filenames = normalizeStringArray(
        entry.filenames || entry.referenceFilenames || entry.reference_filenames || entry.filename,
      );
      ...
      return {
        id: cleanString(entry.id || entry.key || entry.groupId || entry.group_id || label || `dimension-group-${index + 1}`),
        label,
...
function getReferenceEntryIndex(entry = {}, fallbackIndex = 0) {
...
function getProductReferenceEntriesForSku(referenceRoles = []) {
  return (Array.isArray(referenceRoles) ? referenceRoles : [])
    .map((entry, index) => ({ ...entry, referenceIndex: getReferenceEntryIndex(entry, index) }))
...
function buildProductReferenceSkuSubjects(referenceRoles = []) {
  return getProductReferenceEntriesForSku(referenceRoles)
    .map((entry, index) => {
      ...
      const dimensionGroups = normalizeDimensionGroups(entry.dimensionGroups ?? entry.dimension_groups);
      return {
...
function enrichSkuSubjectFromProductReferences(subject = {}, productReferenceSubjects = []) {
  ...
  const structuredColorSource = Array.isArray(subject.colorNames)
    ? subject.colorNames
