...
export function createLightboxImageViewer({ refs, state }) {
  ...
  function syncControls() {
    ...
    refs.lightboxFitButton.disabled = !ready;
    refs.lightboxActualSizeButton.disabled = !ready;
  }
  ...
  function apply() {
    ...
    refs.lightboxImage.style.transform = `translate3d(${panX}, ${panY}, 0) scale(${scale})`;
    refs.lightboxImageShell.classList.toggle("is-viewer-draggable", canPan(current, bounds()));
  ...
  function syncMetrics({ preserveMode = false } = {}) {
    ...
    if (!preserveMode || current.mode === "fit") {
  ...
  function zoomAtPoint(nextScale, anchorPoint = null) {
    ...
    current.scale = scale;
    current.mode = scale <= current.fitScale + LIGHTBOX_VIEWER_SCALE_EPSILON ? "fit" : "inspect";
    if (current.mode === "inspect") {
  ...
  function startPan(event) {
    const current = viewer();
    if (event.button !== 0 || !canPan(current, bounds())) {
      return;
    }
  ...
  function toggleInspectionZoom(anchorPoint = null) {
    ...
    if (current.scale <= current.fitScale + LIGHTBOX_VIEWER_SCALE_EPSILON) {
  ...
  function bindEvents() {
    ...
    refs.lightboxImageShell.addEventListener("wheel", (event) => {
      if (refs.lightbox.classList.contains("hidden") || !hasMetrics(viewer())) {
        return;
      }
      event.preventDefault();
