...
`GPT-Image2-Studio-Setup-v0.2.043.exe` 是 Windows 自解压安装包，由系统自带 `iexpress.exe` 生成。由于 IExpress 构建持续报错，`v0.2.043` GitHub Release 不附带这个兼容包；当前发布请使用 [Windows 桌面程序](./windows-desktop.md) 的 NSIS 安装包或免安装 ZIP。

这个兼容安装包启动后会在默认浏览器中打开工作台。需要独立窗口、任务栏身份和标准 NSIS 卸载体验时，请使用 [Windows 桌面程序](./windows-desktop.md)。

## 安装内容

安装器优先使用系统 `tar.exe` 解压；缺少 tar 时，回退解压要求 [PowerShell 7.4+](https://learn.microsoft.com/powershell/scripting/install/installing-powershell-on-windows) 的 `pwsh.exe`，不会调用 Windows PowerShell 5.1。运行时缺失、版本过低或解压失败均以非零退出码结束。内置 Node 启动入口不要求 PowerShell；附带的源码启动器和 Native Messaging 安装/卸载脚本要求 PowerShell 7.4+。
